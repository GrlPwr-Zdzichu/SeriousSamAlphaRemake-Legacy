...
properties:
 ...
 17 INDEX m_iRocks					"LavaRocks"    'L'   = MAX_ROCKS,
...
functions:
  ...
  BOOL FillEntityStatistics(EntityStats *pes)
  {
    ...
    pes->es_ctAmmount = 1;
    // compile description
    pes->es_strName.PrintF("Back pack: %d Shells, %d Bullets, %d Rockets, %d Grenades, %d Napalm, %d Electricity, %d Iron balls, %d Lava Rocks,",
      m_iShells, m_iBullets, m_iRockets, m_iGrenades, m_iNapalm, m_iElectricity, m_iIronBalls, m_iRocks); 
...
//      m_iShells, m_iBullets, m_iRockets, m_iGrenades, m_iElectricity, m_iIronBalls); 
  ...
  void SetProperties(void)
  {
	...
	if( m_iRocks != 0) {m_strDescription.PrintF("%s: Lava Rocks (%d)", m_strDescription, m_iRocks);}
...
//    eAmmo.iNukeBalls = m_iNukeBalls;
  ...
  Main() {
	...
	m_iRocks = Clamp( m_iRocks, INDEX(0), MAX_ROCKS);
