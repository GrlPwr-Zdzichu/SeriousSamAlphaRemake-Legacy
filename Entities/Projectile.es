...
#include "Models/Weapons/LRG/Projectile/LavaStone.h"
#include "Models/Enemies/Eyeman/Projectile/Acid.h"
...
enum ProjectileType {
 ...
 58 PRT_LAVASTONE			  "LRG",   // Lava Stone for lrg.
 59 PRT_ACID				  "Gnaars Alpha Projectile"  ,   // Gnaars Old projectile
 60 PRT_LAVAMAN_FIRE         "Lavaman",   // stoneman fire rock
 61 PRT_LAVAMAN_BIG_FIRE     "Lavaman Big",   // stoneman big fire rock
 62 PRT_LAVAMAN_LARGE_FIRE   "Lavaman Large",   // stoneman large fire rock
 63 PRT_DEVIL_ALPHA  "Devil Alpha Projectil",   // For alpha ugh zan
};
...
void CProjectile_OnPrecache(CDLLEntityClass *pdec, INDEX iUser) 
{
  ...
  case PRT_DEVIL_ALPHA:         
    
    pdec->PrecacheModel(MODEL_DEVIL_FLARE      );
    pdec->PrecacheTexture(TEXTURE_DEVIL_FLARE  ); 
    break;
  ...
  case PRT_BEAST_BIG_DEBRIS:
    ...
    break;
  case PRT_LAVASTONE:
    pdec->PrecacheModel(MODEL_LAVASTONE      );
    pdec->PrecacheTexture(TEXTURE_LAVASTONE  );
    break;
  case PRT_ACID:
    pdec->PrecacheModel(MODEL_ACID      );
    pdec->PrecacheTexture(TEXTURE_ACID  );
    break;
  case PRT_LAVAMAN_FIRE:
  case PRT_LAVAMAN_BIG_FIRE:
  case PRT_LAVAMAN_LARGE_FIRE:
    pdec->PrecacheModel(MODEL_ELEM_LAVA           );
    pdec->PrecacheTexture(TEXTURE_ELEM_LAVA       ); 
	pdec->PrecacheModel(MODEL_ELEM_LAVA_FLARE           );
    pdec->PrecacheTexture(TEXTURE_ELEM_LAVA_FLARE       );
    break;
  default:
    ASSERT(FALSE);
  }
...
// ********* PLAYER FLAME *********
 ...
 18 sound   SOUND_FLAME         "Sounds\\Fire\\Fire4.wav",
...
// ********* ELEMENTAL FIRE *********
 ...
 87 model   MODEL_ELEM_LAVA			    "Models\\Enemies\\Elementals\\Projectile\\FireBall.mdl",
 88 model   MODEL_ELEM_LAVA_FLARE	    "Models\\Enemies\\Elementals\\Projectile\\LavaStoneFlare.mdl",
 ...
 97 texture TEXTURE_ELEM_LAVA			  "Models\\Enemies\\Elementals\\Projectile\\FireBall.tex",
 98 texture TEXTURE_ELEM_LAVA_FLARE	      "Models\\Enemies\\Elementals\\Projectile\\LavaStoneFlare.tex",
...
133 model   MODEL_GLOW           "Models\\Weapons\\Laser\\Projectile\\LaserProjectileGlow01.mdl",
134 model   MODEL_CYBORG_BOMB       "Models\\Enemies\\Cyborg\\Projectile\\Projectile.mdl",
135 texture TEXTURE_CYBORG_BOMB     "Models\\Enemies\\Cyborg\\Projectile\\Projectile.tex",

// ********* DEVIL ALPHA FIRE *********

170 model   MODEL_DEVIL_FLARE       "Models\\Enemies\\DevilAlpha\\Flare.mdl",
171 texture TEXTURE_DEVIL_FLARE     "Models\\Enemies\\DevilAlpha\\12.tex",


// ********* LAVA STONE *********
142 model   MODEL_LAVASTONE      "Models\\Weapons\\LRG\\Projectile\\LavaStone.mdl",
143 texture TEXTURE_LAVASTONE    "Models\\Weapons\\LRG\\Projectile\\LavaStone.tex",

// ********* GNAARS ACID *********
152 model   MODEL_ACID      "Models\\Enemies\\Eyeman\\Projectile\\Acid.mdl",
153 texture TEXTURE_ACID    "Models\\Enemies\\Eyeman\\Projectile\\Acid.tex",

// ********* BEAST FIRE *********
160 model   MODEL_BEAST_FIRE       "Models\\Enemies\\Beast\\Projectile\\Projectile.mdl",
161 texture TEXTURE_BEAST_FIRE     "Models\\Enemies\\Beast\\Projectile\\Projectile.tex",
162 texture TEXTURE_BEAST_BIG_FIRE "Models\\Enemies\\Beast\\Projectile\\ProjectileBig.tex",
...
void LavaStone(void) {
  ...
  SetCollisionFlags(ECF_PROJECTILE_SOLID);
  SetFlags(GetFlags() | ENF_SEETHROUGH);
  SetModel(MODEL_LAVASTONE);
  GetModelObject()->StretchModel(FLOAT3D(0.65f, 0.65f, 0.65f));
  CModelObject *pmo = GetModelObject();
  if(pmo != NULL)
  {
    //pmo->PlayAnim( LASERPROJECTILE_ANIM_GROW, 0);
  }
  SetModelMainTexture(TEXTURE_LAVASTONE);

  // start moving
  LaunchAsPropelledProjectile(FLOAT3D(0.0f, 0.0f, -120.0f), (CMovableEntity*)(CEntity*)m_penLauncher);
  SetDesiredRotation(ANGLE3D(0, 0, FRnd()*359.0f-1.0f));
  m_fFlyTime = 5.0f;
  m_fDamageAmount = 35.0f;
  m_fSoundRange = 0.0f;
  m_bExplode = TRUE;
  m_bLightSource = TRUE;
  m_bCanHitHimself = FALSE;
...
void LavaRocksGunExplosion(void) {
  ESpawnEffect ese;
  FLOAT3D vPoint;
  FLOATplane3D vPlaneNormal;
  FLOAT fDistanceToEdge;
...
void ElementalRock(INDEX iSize, INDEX iType) {
  ...
  switch (iType) {
    ...
    /*case ELEMENTAL_LAVAMAN:
      SetModel(MODEL_ELEM_LAVA);
      SetModelMainTexture(TEXTURE_ELEM_LAVA);
      AddAttachmentToModel(this, *GetModelObject(), LAVASTONE_ATTACHMENT_FLARE,
        MODEL_ELEM_LAVA_FLARE, TEXTURE_ELEM_LAVA_FLARE, 0, 0, 0);
      break;*/
...
// separate lavaman fire
void ElementalLavaFire(INDEX iSize, INDEX iType) {
  // set appearance
  InitAsModel();
  SetPhysicsFlags(EPF_ONBLOCK_SLIDE|EPF_PUSHABLE|EPF_MOVABLE);
  SetCollisionFlags(ECF_PROJECTILE_SOLID);
...
/************************************************************
 *                   DEVIL ALPHA PROJECTILE                 *
 ************************************************************/
void DevilAlphaProjectile(void) {
  ...
  SetModel(MODEL_DEVIL_FLARE);
  SetModelMainTexture(TEXTURE_DEVIL_FLARE);
  GetModelObject()->StretchModel(FLOAT3D(1.5f, 1.5f, 1.5f));
 ...
 *                   GNAAR PROJECTILE                     *
 ************************************************************/
void GnaarProjectile(void) {
  // set appearance
  InitAsModel();
  SetPhysicsFlags(EPF_PROJECTILE_FLYING);
  SetCollisionFlags(ECF_PROJECTILE_MAGIC);
  SetFlags(GetFlags() | ENF_SEETHROUGH);
  SetComponents(this, *GetModelObject(), MODEL_ACID, TEXTURE_ACID, 0, 0, 0);
...
void CyborgLaser(void) {
  ...
  SetFlags(GetFlags() | ENF_SEETHROUGH);
  SetComponents(this, *GetModelObject(), MODEL_CYBORG_LASER, TEXTURE_CYBORG_LASER, 0, 0, 0);
  GetModelObject()->StretchModel(FLOAT3D(1.35f, 1.35f, 1.35f));
  ModelChangeNotify();
  // start moving
  LaunchAsPropelledProjectile(FLOAT3D(0.0f, 0.0f, -60.0f), (CMovableEntity*)(CEntity*)m_penLauncher);
  SetDesiredRotation(ANGLE3D(0, 0, 0));
...
procedures:
  ...
  Main(ELaunchProjectile eLaunch) {
    ...
    case PRT_WALKER_ROCKET: WalkerRocket(); break;
	  ...
	  case PRT_DEVIL_ALPHA: DevilAlphaProjectile(); break;
	  ...
	  case PRT_LAVASTONE: LavaStone(); break;
	  case PRT_ACID: GnaarProjectile(); break;
	  case PRT_LAVAMAN_FIRE: ElementalLavaFire(ELEMENTAL_NORMAL, ELEMENTAL_LAVAMAN); break;
      case PRT_LAVAMAN_BIG_FIRE: ElementalLavaFire(ELEMENTAL_BIG, ELEMENTAL_LAVAMAN); break;
      case PRT_LAVAMAN_LARGE_FIRE: ElementalLavaFire(ELEMENTAL_LARGE, ELEMENTAL_LAVAMAN); break;
    ...
    switch (m_prtType) {
	  ...
	  case PRT_LAVASTONE: LavaRocksGunExplosion(); break;
    }
