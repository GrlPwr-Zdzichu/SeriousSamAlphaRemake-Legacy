...
// 16/9 fix
extern INDEX ssa_bUseResolutionFix = FALSE; 
extern INDEX ssa_bShowWeaponChange = FALSE;
extern FLOAT hud_fTextScaling = 1.0f;
//extern INDEX ssa_bPure1st = FALSE;  
...
void CPlayer_Precache(void)
{
  ...
  pdec->PrecacheSound(SOUND_FLESH	           );
...
void CPlayer_OnInitClass(void)
{
  ...
  // 16/9 fix
  _pShell->DeclareSymbol("persistent user INDEX ssa_bUseResolutionFix;", &ssa_bUseResolutionFix);
  _pShell->DeclareSymbol("persistent user INDEX ssa_bShowWeaponChange;", &ssa_bShowWeaponChange);
  _pShell->DeclareSymbol("persistent user FLOAT hud_fTextScaling;", &hud_fTextScaling);
  //_pShell->DeclareSymbol("persistent user INDEX ssa_bPure1st;", &ssa_bPure1st);
  //
...
186 sound SOUND_FLESH	        "Sounds\\Weapons\\_BulletHitFlesh.wav",
 ...
 ************************************************************/
  ...
  void RenderDummyView(CDrawPort *pdp)
  {
    ...
    if (!GetSP()->sp_bSinglePlayer) {
	  ...
	  /*if (ssa_bUseResolutionFix) {
	   fScale = (FLOAT)pixDPWidth/820.0f;
	   } else {
	    fScale = (FLOAT)pixDPWidth/640.0f;
		}*/
		fScale = (FLOAT)hud_fTextScaling;
      pdp->SetFont( _pfdDisplayFont);
      pdp->SetTextScaling( fScale);
      pdp->SetTextAspect( 1.0f);
      CTString strMsg;
      strMsg.PrintF(TRANS("%s connected"), GetPlayerName());
      pdp->PutTextCXY( strMsg, pixDPWidth*0.5f, pixDPHeight*0.5f, C_YELLOW|CT_OPAQUE);
    }
  }
  ...
  void RenderPlayerView(CDrawPort *pdp, BOOL bShowExtras)
  {
    ...
    if (_pTimer->CurrentTick()<m_tmCenterMessageEnd) {
      ...
      pdp->SetTextAspect( 1.0f);
	  if (EET_TRIGGER) {
      pdp->PutTextCXY( m_strCenterMessage, pixDPWidth*0.5f, pixDPHeight*0.3f, C_WHITE|0xDD);
	  } else {
    ...
    if (_pTimer->CurrentTick()<m_tmAnalyseEnd) {
      ...
      pdp->PutTextCXY( TRANS("Analyzing..."), pixDPWidth*0.5f, pixDPHeight*0.2f, C_YELLOW|ubA);
    }
  }
  ...
  void RenderCameraView(CDrawPort *pdp, BOOL bListen)
  {
    ...
    _ulPlayerRenderingMask = 1<<GetMyPlayerIndex();
    _ulPlayerRenderingMask = 0;
	if (((CCamera&)*m_penCamera).m_bEpisodicCamera) {
	SetupView(pdp, apr, penViewer, plViewer, colBlend, FALSE);
	RenderView(*en_pwoWorld, *penViewer, apr, *pdp);
	} else {
	...
	// draw interface for main renderer		      
	if (hud_bShowAll && m_penCamera != NULL && !((CCamera&)*m_penCamera).m_bDisableHUD) {
    // let the player entity render its interface
     CPlacement3D plLight(_vViewerLightDirection, ANGLE3D(0,0,0));
     plLight.AbsoluteToRelative(plViewer);
    RenderHUD( *(CPerspectiveProjection3D *)(CProjection3D *)apr, pdp, 
    ...
    if (bListen) {
      ListenFromEntity(penViewer, plViewer);
		...
		// Draw camera render in box, if you marked episodic mode
	   if (((CCamera&)*m_penCamera).m_bEpisodicCamera) {
	CDrawPort dpLeft(  pdp, 0.02, 0.023, 0.42, 0.44f);
 ...
 ************************************************************/
  ...
  void SetUnconnected(void)
  {
    ...
    GetPlayerAnimator()->BodyAnimationTemplate(
      BODY_ANIM_NORMALWALK, BODY_ANIM_KNIFE_STAND, BODY_ANIM_COLT_STAND, BODY_ANIM_SHOTGUN_STAND, BODY_ANIM_MINIGUN_STAND, 
...
//                                        -en_vGravityDir*(fDamageAmmount/15.0f));
	...
	PlaySound(m_soBody, SOUND_FLESH, SOF_3D);
...
//    CPrintF("---APPLY: %g\n", paOriginal.pa_aRotation(1));
  ...
  void AliveActions(const CPlayerAction &pa) 
  {
    ...
    // if camera is active
    if (m_penCamera != NULL && ((CCamera&)*m_penCamera).m_bDisablePlayerControls) {
 ...
 ************************************************************/
  Death(EDeath eDeath) {
    ...
    if (m_pstState == PST_SWIM || m_pstState == PST_DIVE) {
      ...
      iAnim2 = BODY_ANIM_BODY_DEATH_UNDERWATER;
    } else if (eDeath.eLastDamage.dmtType==DMT_SPIKESTAB) {
      iAnim1 = PLAYER_ANIM_DEATH_SPIKES;
      iAnim2 = BODY_ANIM_BODY_DEATH_SPIKES;
    } else if (eDeath.eLastDamage.dmtType==DMT_ABYSS) {
      iAnim1 = PLAYER_ANIM_DEATH01;
      iAnim2 = BODY_ANIM_BODY_DEATH_NORMAL;
    } else {
      FLOAT3D vFront;
 ...
 ************************************************************/
  ...
  AutoGoToMarker(EVoid)
  {
    ...
    if (m_fAutoSpeed>plr_fSpeedForward/2) {
      ...
      StartModelAnim(PLAYER_ANIM_WALK, ulFlags);
    }
    ...
    // auto action - swim to current marker
  AutoSwimToMarker(EVoid)
  {
    ULONG ulFlags = AOF_LOOPING|AOF_NORESTART;

    INDEX iAnim = GetModelObject()->GetAnim();
    if( iAnim!=PLAYER_ANIM_SWIMIDLE)
    {
      ulFlags |= AOF_SMOOTHCHANGE;
    }

    CPlayerAnimator &plan = (CPlayerAnimator&)*m_penAnimator;
    plan.BodySwimmingAnimation();
    if (m_fAutoSpeed>plr_fSpeedForward/2) {
      StartModelAnim(PLAYER_ANIM_SWIM, ulFlags);
    } else {
      StartModelAnim(PLAYER_ANIM_SWIM, ulFlags);
    }
  ...
  AutoGoToMarkerAndStop(EVoid)
  {
    ...
    // auto action - swim to current marker and stop there
  AutoSwimToMarkerAndStop(EVoid)
  {
    ULONG ulFlags = AOF_LOOPING|AOF_NORESTART;
  ...
  AutoFallDown(EVoid)
  {
    StartModelAnim(PLAYER_ANIM_JUMPSTART, 0);
    CModelObject &moBody = GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject;
    moBody.PlayAnim(BODY_ANIM_COLT_JUMP_START, 0);
  ...
  {
    StartModelAnim(PLAYER_ANIM_DEATH01, AOF_LOOPING);
    CModelObject &moBody = GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject;
    moBody.PlayAnim(BODY_ANIM_BODY_DEATH_NORMAL, AOF_LOOPING);
  ...
  {
    StartModelAnim(PLAYER_ANIM_WALKBACK, 0);
  ...
  AutoAppear(EVoid)
  {
    ...
    StartModelAnim(PLAYER_ANIM_DEATH01, AOF_NORESTART);
    CModelObject &moBody = GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject;
    moBody.PlayAnim(BODY_ANIM_BODY_DEATH_NORMAL, AOF_NORESTART);
    ...
    StartModelAnim(PLAYER_ANIM_JUMPEND, AOF_SMOOTHCHANGE);
    CModelObject &moBody = GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject;
    moBody.PlayAnim(BODY_ANIM_COLT_JUMP_END, AOF_SMOOTHCHANGE);
  ...
  EnteringPortal()
  {
    ...
    // make it rotate in spawnpose
	    StartModelAnim(PLAYER_ANIM_JUMPSTART, AOF_NORESTART);
		    CModelObject &moBody = GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject;
    moBody.PlayAnim(BODY_ANIM_KNIFE_JUMP_START, AOF_NORESTART);
    SetPhysicsFlags(GetPhysicsFlags() & ~(EPF_TRANSLATEDBYGRAVITY|EPF_ORIENTEDBYGRAVITY));
    m_ulFlags|=PLF_AUTOMOVEMENTS;
    SetDesiredTranslation(ANGLE3D(0,0.5f,0));
	SetDesiredRotation(ANGLE3D(0,15,0));
    // wait till it appears
    // return to auto-action loop
    return EReturn();
  }
  ...
  LogoFireMinigun(EVoid) 
  {
    ...
    if( penBcgViewer != NULL)
    {
      ...
      pwsc->m_fShakeFalloff = 0.0f;
      pwsc->m_fShakeFade = 0.0f;

      pwsc->m_fShakeIntensityZ = 0.0f;
      pwsc->m_tmShakeFrequencyZ = 0.0f;
  ...
  AutoStoreWeapon(EVoid) 
  {
    ...
    plan.BodyAnimationTemplate(BODY_ANIM_WAIT, 
      BODY_ANIM_KNIFE_PUSH_SLOW, BODY_ANIM_COLT_PUSH_SLOW, BODY_ANIM_SHOTGUN_PUSH_SLOW, BODY_ANIM_MINIGUN_PUSH_SLOW, 
      0);
    ...
    plan.BodyAnimationTemplate(BODY_ANIM_WAIT, 
      BODY_ANIM_PUSHTOWALKKNIFE, BODY_ANIM_PUSHTOWALKCOLT, BODY_ANIM_PUSHTOWALK, BODY_ANIM_PUSHTOWALK, AOF_SMOOTHCHANGE);
    ((CPlayerWeapons&)*m_penWeapons).m_iCurrentWeapon = WEAPON_NONE;
    ...
    while (m_penActionMarker!=NULL && IsOfClass(m_penActionMarker, "PlayerActionMarker")) {
      ...
      // if should teleport here
        ...
        autocall TravellingInBeam() EReturn;
		      } else if (GetActionMarker()->m_paaAction==PAA_PORTALANIM) {
        autocall EnteringPortal() EReturn;
      ...
      // if should run to the marker
        ...
        autocall AutoGoToMarker() EReturn;
		      ...
		      // if should swim to the marker
      } else if (GetActionMarker()->m_paaAction==PAA_RUN) {
        // go to it
        m_fAutoSpeed = plr_fSpeedForward*GetActionMarker()->m_fSpeed;                                             
        autocall AutoSwimToMarker() EReturn;
      ...
      // if should run to the marker and stop exactly there
        ...
        autocall AutoGoToMarkerAndStop() EReturn;
		      ...
		      // if should run to the marker and stop exactly there
      } else if (GetActionMarker()->m_paaAction==PAA_SWIMSTOP) {
        // go to it
        m_fAutoSpeed = plr_fSpeedForward*GetActionMarker()->m_fSpeed;                                             
        autocall AutoSwimToMarkerAndStop() EReturn;
