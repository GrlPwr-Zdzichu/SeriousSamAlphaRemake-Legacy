...
enum EyemanEnv {
  0 EYE_NORMAL    "Normal",  1 EYE_LAVA      "Lava",
...
components:
  ...
  6 model   MODEL_ACID      "Models\\Enemies\\Eyeman\\Projectile\\Acid.mdl",
  7 texture TEXTURE_ACID    "Models\\Enemies\\Eyeman\\Projectile\\Acid.tex",
...
// ************** SOUNDS ************** 
 ...
 57 sound   SOUND_FIRE      "Models\\Enemies\\Eyeman\\Sounds\\Fire.wav",
...
functions:
  ...
  void Precache(void) {
	...
	PrecacheModel(MODEL_ACID);
	PrecacheTexture(TEXTURE_ACID);
  };
 ...
 ************************************************************/
  ...
  Fire(EVoid) : CEnemyBase::Fire { 
    if (!m_bInAir) {
    // fire projectile
    StartModelAnim(EYEMAN_ANIM_ATTACK01, 0);
	StopMoving();
	autowait(GetModelObject()->GetCurrentAnimLength()/1.1);
	ShootProjectile(PRT_ACID, FLOAT3D(0.0f, 1.5f, 0.5f), ANGLE3D(0, 0, 0));
