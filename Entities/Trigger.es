...
procedures:
  SendEventToTargets() {
    // if needed wait some time before event is send
		  		CTString strName = m_strName;
	CPrintF("Activated trigger: %s %\n", strName);
    if (m_fWaitTime > 0.0f) {
      wait (m_fWaitTime) {
        on (EBegin) : { resume; }
        on (ETimer) : { stop; }
	...
	CPrintF("Target1: %s %\n", strTargetName, m_penTarget1);
	CPrintF("Target2: %s %\n", strTargetName, m_penTarget2);
	CPrintF("Target3: %s %\n", strTargetName, m_penTarget3);
	CPrintF("Target4: %s %\n", strTargetName, m_penTarget4);
	CPrintF("Target5: %s %\n", strTargetName, m_penTarget5);
	CPrintF("Target6: %s %\n", strTargetName, m_penTarget6);
	CPrintF("Target7: %s %\n", strTargetName, m_penTarget7);
	CPrintF("Target8: %s %\n", strTargetName, m_penTarget8);
	CPrintF("Target9: %s %\n", strTargetName, m_penTarget9);
	CPrintF("Target10: %s %\n", strTargetName, m_penTarget10);*/
    ...
    if (m_fScore>0) {
      CEntity *penCaused = FixupCausedToPlayer(this, m_penCaused);
	  	INDEX i_rcvscore = m_fScore;
		CPrintF("Set %d score, received %d score %\n", i_rcvscore, i_rcvscore);
