...
enum KeyItemType {
 ...
 20 KIT_WCARD				"White Keycard",
 21 KIT_BCARD				"Black Keycard",
};
...
const char *GetKeyName(enum KeyItemType kit)
{
  ...
  case KIT_WCARD       :  return TRANS("White Keycard"); break;
  case KIT_BCARD       :  return TRANS("Black Keycard"); break;
...
// ********* ANKH KEY *********
 ...
 100 model   MODEL_CARD     "Models\\Items\\Keys\\KeyCard\\KeyCard.mdl",
 101 texture TEXTURE_WCARD    "Models\\Items\\Keys\\KeyCard\\White.tex",
 102 texture TEXTURE_BCARD    "Models\\Items\\Keys\\KeyCard\\Black.tex",
...
260 texture TEX_REFL_RIPLES     "Models\\SpecularTextures\\BWRiples02.tex",
260 texture TEX_REFL_METAL02    "Models\\SpecularTextures\\DarkMetal.tex",
...
functions:
  ...
  void SetProperties(void) {
    ...
    switch (m_kitType) {
	  ...
	  case KIT_WCARD:
        // set appearance
        AddItem(MODEL_CARD, TEXTURE_WCARD, TEX_REFL_RIPLES, TEX_SPEC_MEDIUM, 0);
        // add flare
        AddFlare(MODEL_FLARE, TEXTURE_FLARE, FLOAT3D(0,0.2f,0), FLOAT3D(1,1,0.3f) );
        StretchItem(FLOAT3D(1.0f, 1.0f, 1.0f));
        m_iSoundComponent = SOUND_KEY;
	  ...
	  case KIT_BCARD:
        // set appearance
        AddItem(MODEL_CARD, TEXTURE_BCARD, TEX_REFL_METAL02, TEX_SPEC_MEDIUM, 0);
        // add flare
        AddFlare(MODEL_FLARE, TEXTURE_FLARE, FLOAT3D(0,0.2f,0), FLOAT3D(1,1,0.3f) );
        StretchItem(FLOAT3D(1.0f, 1.0f, 1.0f));
        m_iSoundComponent = SOUND_KEY;
