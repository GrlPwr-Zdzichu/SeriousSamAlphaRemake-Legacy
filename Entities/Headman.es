...
#define CHAINSAW_DMG  2.5f
#define CHAINSAW_ATTACK  1.0f
%}
...
components:
  ...
  9 model   MODEL_HEADONSTICK      "Models\\Enemies\\Headman\\HeadOnStick.mdl",
 ...
 16 model   MODEL_BAG			  "Models\\Enemies\\Headman\\Bag.mdl",
 17 model   MODEL_BOMB            "Models\\Enemies\\Headman\\Projectile\\Bomb.mdl",
 18 model   MODEL_STICK           "Models\\Enemies\\Headman\\Stick.mdl",
 19 model   MODEL_SAW             "Models\\Enemies\\Headman\\Saw.mdl",
 ...
 30 texture TEXTURE_STICK           "Models\\Enemies\\Headman\\Stick.tex",
 31 texture TEXTURE_SAW             "Models\\Enemies\\Headman\\Saw.tex",
 32 texture TEXTURE_BAG             "Models\\Enemies\\Headman\\Bag.tex",
 33 texture TEXTURE_HEADONSTICK     "Models\\Enemies\\Headman\\BombermanHead.tex",
...
// ************** SOUNDS **************
 ...
 59 sound   SOUND_CHAINSAW          "Models\\Enemies\\Headman\\Sounds\\ChainsawAttack.wav",
...
functions:
  ...
  void Precache(void) {
    ...
    case HDT_BOMBERMAN:   {  
	  ...
	  PrecacheModel(MODEL_HEADONSTICK);
      PrecacheTexture(TEXTURE_HEADONSTICK);
	  PrecacheModel(MODEL_BAG);
      PrecacheTexture(TEXTURE_BAG);    
                          } break;
    case HDT_KAMIKAZE:    { 
  ...
  INDEX AnimForDamage(FLOAT fDamage) {
    INDEX iAnim;
    if (IRnd()%2) {
      iAnim = HEADMAN_ANIM_WOUND01;
    } else {
      iAnim = HEADMAN_ANIM_WOUND02;
    }
  ...
  INDEX AnimForDeath(void) {
    ...
    if (fDamageDir<0) {
      if (Abs(fDamageDir)<10.0f) {
        iAnim = HEADMAN_ANIM_DEATH01;
      } else {
        iAnim = HEADMAN_ANIM_DEATH02;
      }
    } else {
      if (Abs(fDamageDir)<10.0f) {
        iAnim = HEADMAN_ANIM_DEATH03;
      } else {
        iAnim = HEADMAN_ANIM_MACRODEATH05;
      }
    }
  ...
  void StandingAnim(void) {
    StartModelAnim(HEADMAN_ANIM_IDLE01, AOF_LOOPING|AOF_NORESTART);
  ...
  void WalkingAnim(void) {
    StartModelAnim(HEADMAN_ANIM_WALK01, AOF_LOOPING|AOF_NORESTART);
  };
  void RunningAnim(void) {
    if (m_hdtType==HDT_KAMIKAZE) {
      ...
      StartModelAnim(HEADMAN_ANIM_RUNFIRE03, AOF_LOOPING|AOF_NORESTART);
    }
  };
 ...
 ************************************************************/
  ...
  FirecrackerAttack(EVoid) {
	...
	// enemy is away from defined distance
	    if (CalcDist(m_penEnemy) > CHAINSAW_DMG) {
    ...
    // enemy is in defined/closer distance, use melee attack
	StartModelAnim(HEADMAN_ANIM_CHAINSAW_ATTACK, 0);
	PlaySound(m_soSound, SOUND_CHAINSAW, SOF_3D);
	StopMoving();
  ...
  Main(EVoid) {
    ...
    switch (m_hdtType) {
      case HDT_FIRECRACKER:
        ...
        AddAttachment(HEADMAN_ATTACHMENT_HEAD, MODEL_HEAD, TEXTURE_FIRECRACKERHEAD);
        AddAttachment(HEADMAN_ATTACHMENT_CHAINSAW, MODEL_CHAINSAW, TEXTURE_CHAINSAW);
		AddAttachment(HEADMAN_ATTACHMENT_STICK, MODEL_STICK, TEXTURE_STICK);
        // setup moving speed
      ...
      case HDT_ROCKETMAN:
        ...
        AddAttachment(HEADMAN_ATTACHMENT_CHAINSAW, MODEL_ROCKETLAUNCHER, TEXTURE_ROCKETLAUNCHER);
		AddAttachment(HEADMAN_ATTACHMENT_SAW, MODEL_SAW, TEXTURE_SAW);
        // setup moving speed
        m_fWalkSpeed = FRnd() + 1.5f;
        m_aWalkRotateSpeed = AngleDeg(FRnd()*10.0f + 500.0f);
      ...
      case HDT_BOMBERMAN:
        ...
        //AddAttachment(HEADMAN_ATTACHMENT_HEAD, MODEL_HEAD, TEXTURE_HEAD);
		AddAttachment(HEADMAN_ATTACHMENT_HEAD, MODEL_BAG, TEXTURE_BAG);
		AddAttachment(HEADMAN_ATTACHMENT_SAW, MODEL_HEADONSTICK, TEXTURE_HEADONSTICK);
        // setup moving speed
        m_fWalkSpeed = FRnd() + 1.5f;
        m_aWalkRotateSpeed = AngleDeg(FRnd()*10.0f + 500.0f);
