...
void CPlayerAnimator_Precache(ULONG ulAvailable)
{
  ...
  if ( ulAvailable&(1<<(WEAPON_KNIFE-1)) ) {
	...
	pdec->PrecacheModel(MODEL_KNIFEFORPLAYER        );
	pdec->PrecacheTexture(TEXTURE_KNIFEFORPLAYER    );
...
// ************** KNIFE **************
 ...
 21 model   MODEL_KNIFEFORPLAYER        "Models\\Weapons\\Knife\\KnifeForPlayer.mdl",
 22 texture TEXTURE_KNIFE               "Models\\Weapons\\Knife\\KnifeItem.tex",
 23 texture TEXTURE_KNIFEFORPLAYER      "Models\\Weapons\\Knife\\KnifeItem.tex",
...
172 texture TEXTURE_CANNON              "Models\\Weapons\\Cannon\\Body.tex",
173 model   MODEL_CN_FUSE            "Models\\Weapons\\Cannon\\Fuse.mdl",
174 texture   TEXTURE_FUSE              "Models\\Weapons\\Cannon\\Fuse.tex",

// ************** LAVA ROCKS **************
180 model   MODEL_LRG                 "Models\\Weapons\\LRG\\LRGItem.mdl",
...
// ************** AMON STATUE **************
190 model   MODEL_GOLDAMON                "Models\\Ages\\Egypt\\Gods\\Amon\\AmonGold.mdl",
191 texture TEXTURE_GOLDAMON              "Models\\Ages\\Egypt\\Gods\\Amon\\AmonGold.tex",
...
functions:
  ...
  void SyncWeapon(void)
  {
    ...
    // Liczba Attachmentow w body
    for (INDEX iWeapon = BODY_ATTACHMENT_GHOSTBUSTER; iWeapon<=BODY_ATTACHMENT_LRG; iWeapon++) {
  ...
  void SetWeapon(void) {
    ...
    // *********** KNIFE ***********
      case WEAPON_KNIFE:
        AddWeaponAttachment(BODY_ATTACHMENT_KNIFE, MODEL_KNIFEFORPLAYER,
                            TEXTURE_KNIFEFORPLAYER, TEX_REFL_BWRIPLES02, TEX_SPEC_WEAK, 0);
        break;
    ...
    // *********** GRENADE LAUNCHER ***********
      case WEAPON_GRENADELAUNCHER:
        ...
        AddWeaponAttachment(GRENADELAUNCHERITEM_ATTACHMENT_MAIN_PART, MODEL_GL_MAIN_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);                   
        AddWeaponAttachment(GRENADELAUNCHERITEM_ATTACHMENT_SECOND_PART, MODEL_GL_SECOND_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddWeaponAttachment(GRENADELAUNCHERITEM_ATTACHMENT_THRID_PART, MODEL_GL_THRID_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);  
		AddWeaponAttachment(GRENADELAUNCHERITEM_ATTACHMENT_LAST_PART, MODEL_GL_LAST_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0); 
        break;

    // *********** PIPEBOMB ***********
      case WEAPON_PIPEBOMB:
        AddWeaponAttachment(BODY_ATTACHMENT_PIPEBOMB_DETONATOR, MODEL_PIPEBOMB_STICK, TEXTURE_PB_STICK, 0, 0, 0);
        SetAttachment(BODY_ATTACHMENT_PIPEBOMB_DETONATOR);
    ...
    // *********** GHOSTBUSTER ***********
      case WEAPON_GHOSTBUSTER: {
        AddWeaponAttachment(BODY_ATTACHMENT_GHOSTBUSTER, MODEL_GHOSTBUSTER, TEXTURE_GB_BODY, 0, 0, 0);
        SetAttachment(BODY_ATTACHMENT_GHOSTBUSTER);
    ...
    // *********** CANNON ***********
      case WEAPON_IRONCANNON:
        ...
        AddWeaponAttachment(CANNON_ATTACHMENT_FUSE, MODEL_CN_FUSE, TEXTURE_FUSE, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        break;

   // *********** LAVA ROCKS ***********
      case WEAPON_LRG:
        AddWeaponAttachment(BODY_ATTACHMENT_LRG, MODEL_LRG, TEXTURE_BODY, 0, 0, 0);
        SetAttachment(BODY_ATTACHMENT_LRG);
        AddWeaponAttachment(LRGITEM_ATTACHMENT_BODY, MODEL_LR_BODY, TEXTURE_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddWeaponAttachment(LRGITEM_ATTACHMENT_BARREL, MODEL_LR_BARREL, TEXTURE_BARREL, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
		break;
    }
    // sync apperances
    SyncWeapon();
 ...
 ************************************************************/
  ...
  void AnimateSoftEyes(void) {
    ...
    m_fEyesYOffset -= Clamp(m_fEyesYOffset, 0.1f, fRelY);
    m_fWeaponYOffset -= ClampUp(fRelY, 0.0f);

    plr_fViewDampFactor      = Clamp(plr_fViewDampFactor      ,0.0f,1.0f);
    plr_fViewDampLimitGroundUp = Clamp(plr_fViewDampLimitGroundUp ,0.35f,2.0f);
    plr_fViewDampLimitGroundDn = Clamp(plr_fViewDampLimitGroundDn ,0.0f,2.0f);
    plr_fViewDampLimitWater  = Clamp(plr_fViewDampLimitWater  ,0.25f,2.0f);
        ...
        // not in jump anim and in stand mode change
          ...
          if (!m_bCrouch) {
            ...
            if (vDesiredTranslation.Length()>5.0f && vCurrentTranslation.Length()>5.0f) {
              if (vCurrentTranslation(3)<0) {
                ...
                pl.StartModelAnim(PLAYER_ANIM_RUNBACK, AOF_LOOPING|AOF_NORESTART);
				BodyWalkAnimation();
              }
              BodyStillAnimation();
              m_fLastActionTime = _pTimer->CurrentTick();
            // walking anim
              ...
              if (vCurrentTranslation(3)<0) {
                pl.StartModelAnim(PLAYER_ANIM_WALK, AOF_LOOPING|AOF_NORESTART);
              } else {
                pl.StartModelAnim(PLAYER_ANIM_WALKBACK, AOF_LOOPING|AOF_NORESTART);
              }
              BodyWalkAnimation();
              m_fLastActionTime = _pTimer->CurrentTick();
            // left rotation anim
          ...
          // crouch
            ...
            if (vDesiredTranslation.Length()>2.0f && vCurrentTranslation.Length()>2.0f) {
              if (vCurrentTranslation(3)<0) {
                pl.StartModelAnim(PLAYER_ANIM_CROUCHWALK, AOF_LOOPING|AOF_NORESTART);
              } else {
                pl.StartModelAnim(PLAYER_ANIM_CROUCHWALKBACK, AOF_LOOPING|AOF_NORESTART);
              }
              BodyCrouchWalkAnimation();
              m_fLastActionTime = _pTimer->CurrentTick();
            // left rotation anim
            } else if (aDesiredRotation(1)>0.5f) {
              pl.StartModelAnim(PLAYER_ANIM_CROUCHTURNLEFT, AOF_LOOPING|AOF_NORESTART);
              BodyCrouchWalkAnimation();
              m_fLastActionTime = _pTimer->CurrentTick();
            // right rotation anim
            } else if (aDesiredRotation(1)<-0.5f) {
              pl.StartModelAnim(PLAYER_ANIM_CROUCHTURNRIGHT, AOF_LOOPING|AOF_NORESTART);
              BodyCrouchWalkAnimation();
              m_fLastActionTime = _pTimer->CurrentTick();
            // standing anim
            } else {
              pl.StartModelAnim(PLAYER_ANIM_CROUCHIDLE, AOF_LOOPING|AOF_NORESTART);
              BodyStillAnimation();
            }
          }
    ...
    if (m_bSwim) {
      ...
      switch (iWeapon) {
        case WEAPON_NONE:
          break;
		case WEAPON_KNIFE:
          iAnim += BODY_ANIM_KNIFE_SWIMMING-BODY_ANIM_KNIFE_SWIMMING;
          break;
        case WEAPON_COLT: case WEAPON_DOUBLECOLT: case WEAPON_PIPEBOMB:
          iAnim += BODY_ANIM_COLT_SWIMMING-BODY_ANIM_COLT_SWIMMING;
          break;
        case WEAPON_SINGLESHOTGUN: case WEAPON_DOUBLESHOTGUN: case WEAPON_TOMMYGUN: 
        case WEAPON_FLAMER: case WEAPON_GHOSTBUSTER:
          iAnim += BODY_ANIM_SHOTGUN_SWIMMING-BODY_ANIM_SHOTGUN_SWIMMING;
          break;
        case WEAPON_MINIGUN: case WEAPON_ROCKETLAUNCHER: case WEAPON_GRENADELAUNCHER: case WEAPON_LRG:
        case WEAPON_IRONCANNON: case WEAPON_LASER:
          iAnim += BODY_ANIM_MINIGUN_SWIMMING-BODY_ANIM_MINIGUN_SWIMMING;
          break;
      }
    }
 ...
 ************************************************************/
  ...
  void BodyAnimationTemplate(INDEX iNone, INDEX iKnife, INDEX iColt, INDEX iShotgun, INDEX iMinigun, ULONG ulFlags) {
    INDEX iWeapon = ((CPlayerWeapons&)*(((CPlayer&)*m_penPlayer).m_penWeapons)).m_iCurrentWeapon;
    switch (iWeapon) {
      case WEAPON_NONE:
        ...
        break;
	  case WEAPON_KNIFE:
        if (m_bSwim) { iKnife += BODY_ANIM_KNIFE_SWIMMING-BODY_ANIM_KNIFE_SWIMMING; }
        SetBodyAnimation(iKnife, ulFlags);
        break;
      case WEAPON_COLT: case WEAPON_DOUBLECOLT:  case WEAPON_PIPEBOMB:
        if (m_bSwim) { iColt += BODY_ANIM_COLT_SWIMMING-BODY_ANIM_COLT_SWIMMING; }
        SetBodyAnimation(iColt, ulFlags);
        break;
      case WEAPON_SINGLESHOTGUN: case WEAPON_DOUBLESHOTGUN: case WEAPON_TOMMYGUN: 
      case WEAPON_FLAMER: case WEAPON_GHOSTBUSTER:
        if (m_bSwim) { iShotgun += BODY_ANIM_SHOTGUN_SWIMMING-BODY_ANIM_SHOTGUN_SWIMMING; }
        SetBodyAnimation(iShotgun, ulFlags);
        break;
      case WEAPON_MINIGUN: case WEAPON_ROCKETLAUNCHER: case WEAPON_GRENADELAUNCHER:  case WEAPON_LRG:
      case WEAPON_IRONCANNON: case WEAPON_LASER: 
        if (m_bSwim) { iMinigun+=BODY_ANIM_MINIGUN_SWIMMING-BODY_ANIM_MINIGUN_SWIMMING; }
  ...
  void BodyWalkAnimation() {
    BodyAnimationTemplate(BODY_ANIM_NORMALWALK, 
     BODY_ANIM_KNIFE_WALKRUN, BODY_ANIM_COLT_WALKRUN, BODY_ANIM_SHOTGUN_WALKRUN, BODY_ANIM_MINIGUN_WALKRUN, 
  ...
  void BodyJumpStartAnimation() {
    BodyAnimationTemplate(BODY_ANIM_NORMALWALK, 
     BODY_ANIM_KNIFE_JUMP_START, BODY_ANIM_COLT_JUMP_START, BODY_ANIM_SHOTGUN_JUMP_START, BODY_ANIM_MINIGUN_JUMP_START, 
      ...
      // End Jumping
  void BodyJumpEndAnimation() {
    BodyAnimationTemplate(BODY_ANIM_NORMALWALK, 
     BODY_ANIM_KNIFE_JUMP_END, BODY_ANIM_COLT_JUMP_END, BODY_ANIM_SHOTGUN_JUMP_END, BODY_ANIM_MINIGUN_JUMP_END, 
      AOF_NORESTART|AOF_NORESTART);
  };
  ...
  void BodyStillAnimation() {
    BodyAnimationTemplate(BODY_ANIM_WAIT, 
     BODY_ANIM_KNIFE_STAND, BODY_ANIM_COLT_STAND, BODY_ANIM_SHOTGUN_STAND, BODY_ANIM_MINIGUN_STAND, 
      AOF_LOOPING|AOF_NORESTART);
  };

    // crouch walk
  void BodyCrouchWalkAnimation() {
    BodyAnimationTemplate(BODY_ANIM_WAIT, 
     BODY_ANIM_KNIFE_CROUCHWALK, BODY_ANIM_COLT_CROUCHWALK, BODY_ANIM_SHOTGUN_CROUCHWALK, BODY_ANIM_MINIGUN_CROUCHWALK, 
      AOF_LOOPING|AOF_NORESTART);
  };

    // turning
  void BodyTurningAnimation() {
    BodyAnimationTemplate(BODY_ANIM_WAIT, 
     BODY_ANIM_KNIFE_TURNING, BODY_ANIM_COLT_TURNING, BODY_ANIM_SHOTGUN_TURNING, BODY_ANIM_MINIGUN_TURNING, 
      AOF_LOOPING|AOF_NORESTART);
	};

	    // turning
  void BodySwimmingAnimation() {
    BodyAnimationTemplate(BODY_ANIM_WAIT, 
     BODY_ANIM_KNIFE_SWIMMING, BODY_ANIM_COLT_SWIMMING, BODY_ANIM_SHOTGUN_SWIMMING, BODY_ANIM_MINIGUN_SWIMMING, 
      AOF_LOOPING|AOF_NORESTART);
	};
  ...
  void BodyPushAnimation() {
    ...
    m_bChangeWeapon = FALSE;
    BodyAnimationTemplate(BODY_ANIM_WAIT, 
      BODY_ANIM_KNIFE_PUSH, BODY_ANIM_COLT_PUSH, BODY_ANIM_SHOTGUN_PUSH, BODY_ANIM_MINIGUN_PUSH, 0);
  ...
  void RemoveWeapon(void) 
  {
    ...
    switch (m_iWeaponLast) {
      ...
      case WEAPON_GRENADELAUNCHER:
        ...
        break;
      case WEAPON_PIPEBOMB:
        pmoModel->RemoveAttachmentModel(BODY_ATTACHMENT_PIPEBOMB_DETONATOR);
        // reset to player body
        pmoModel = &(pl.GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject);
        pmoModel->RemoveAttachmentModel(BODY_ATTACHMENT_COLT_LEFT);
        break;
      ...
      case WEAPON_LASER:
        ...
        break;
      case WEAPON_GHOSTBUSTER:
        pmoModel->RemoveAttachmentModel(BODY_ATTACHMENT_GHOSTBUSTER);
        break;
      case WEAPON_IRONCANNON:
//      case WEAPON_NUKECANNON:
        ...
        break;
	  case WEAPON_LRG:
        pmoModel->RemoveAttachmentModel(BODY_ATTACHMENT_LRG);
        break;
      default:
        ASSERT(FALSE);
    }
  ...
  void BodyPullAnimation() {
    ...
    m_bChangeWeapon = FALSE;
    BodyAnimationTemplate(BODY_ANIM_WAIT, 
     BODY_ANIM_KNIFE_PULL, BODY_ANIM_COLT_PULL, BODY_ANIM_SHOTGUN_PULL, BODY_ANIM_MINIGUN_PULL, 0);
    INDEX iWeapon = ((CPlayerWeapons&)*(((CPlayer&)*m_penPlayer).m_penWeapons)).m_iCurrentWeapon;
  ...
  void BodyPickItemAnimation() {
    ...
    // pick item
    m_bChangeWeapon = FALSE;
    SetBodyAnimation(BODY_ANIM_WAIT, 0);
    m_bChangeWeapon = TRUE;
    SpawnReminder(this, m_fBodyAnimTime, (INDEX) AA_PULLWEAPON);
    // sync apperances
    SyncWeapon();
