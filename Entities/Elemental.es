...
enum ElementalType {
  0 ELT_AIR           "AirMan",        // air elemental
  1 ELT_ICE           "IceMan",        // ice elemental
  2 ELT_LAVA          "LavaMan",       // lava elemental
  3 ELT_STONE         "StoneMan",      // stone elemental
  4 ELT_WATER         "WaterMan",      // water elemental
};
...
components:
 ...
 15 texture TEXTURE_AIR_F             "Textures\\Effects\\Flares\\03\\Flaire06.tex",
 ...
 // lava
 30 model   MODEL_LAVA              "Models\\Enemies\\Elementals\\ElementalLava.mdl",
 31 model   MODEL_LAVA_BODY_FLARE   "Models\\Enemies\\Elementals\\BodyFlare.mdl",
 32 model   MODEL_LAVA_HAND_FLARE   "Models\\Enemies\\Elementals\\HandFlare.mdl",
 33 texture TEXTURE_LAVA            "Models\\Enemies\\Elementals\\Lava04Fx.tex",
 34 texture TEXTURE_LAVA_DETAIL     "Models\\Enemies\\Elementals\\LavaMan01.tex",
 35 texture TEXTURE_LAVA_FLARE      "Models\\Enemies\\Elementals\\Flare.tex",
 ...
 54 model   MODEL_WATER_DROP_PROJECTIVE "Models\\Enemies\\Elementals\\Projectile\\WaterDrop.mdl",
...
// ************** SOUNDS **************
250 sound   SOUND_LAVA_IDLE      "Models\\Enemies\\Elementals\\Sounds\\Idle.wav",
252 sound   SOUND_LAVA_WOUND     "Models\\Enemies\\Elementals\\Sounds\\Wound.wav",
253 sound   SOUND_LAVA_FIRE      "Models\\Enemies\\Elementals\\Sounds\\Fire.wav",
254 sound   SOUND_LAVA_KICK      "Models\\Enemies\\Elementals\\Sounds\\Kick.wav",
255 sound   SOUND_LAVA_DEATH     "Models\\Enemies\\Elementals\\Sounds\\Death.wav",
//220 sound   SOUND_LAVA_LAVABURN  "Models\\Enemies\\Elementals\\Sounds\\LavaBurn.wav",
221 sound   SOUND_LAVA_ANGER     "Models\\Enemies\\Elementals\\Sounds\\Anger.wav",
222 sound   SOUND_LAVA_GROW      "Models\\Enemies\\Elementals\\Sounds\\Sight.wav",

260 sound   SOUND_IDLE      	 "Models\\Enemies\\Elementals\\Sounds\\Idle.wav",
262 sound   SOUND_WOUND     	 "Models\\Enemies\\Elementals\\Sounds\\Wound.wav",
263 sound   SOUND_FIRE      	 "Models\\Enemies\\Elementals\\Sounds\\Fire.wav",
264 sound   SOUND_KICK        	 "Models\\Enemies\\Elementals\\Sounds\\Kick.wav",
265 sound   SOUND_DEATH     	 "Models\\Enemies\\Elementals\\Sounds\\Death.wav",
...
functions:
  ...
  virtual CTString GetPlayerKillDescription(const CTString &strPlayerName, const EDeath &eDeath)
  {
	...
	if (m_EetType == ELT_AIR) {
        str.PrintF(TRANS("%s was killed by a Air Golem"), strPlayerName);
        return str;
	} else if (m_EetType == ELT_ICE){
        str.PrintF(TRANS("%s was killed by a Ice Golem"), strPlayerName);
        return str;
	} else if (m_EetType == ELT_LAVA){
        str.PrintF(TRANS("%s was killed by a Lava Golem"), strPlayerName);
        return str;
	} else if (m_EetType == ELT_STONE){
        str.PrintF(TRANS("%s was killed by a Stone Golem"), strPlayerName);
        return str;
	} else if (m_EetType ==  ELT_WATER){
        str.PrintF(TRANS("%s was killed by a Water Golem"), strPlayerName);
        return str;
    }
  ...
  virtual const CTFileName &GetComputerMessageName(void) const {
	if (m_EetType == ELT_AIR) {
        static DECLARE_CTFILENAME(fnm, "Data\\Messages\\Enemies\\ElementalAir.txt");
        return fnm;
	} else if (m_EetType == ELT_ICE){
        static DECLARE_CTFILENAME(fnm, "Data\\Messages\\Enemies\\ElementalIce.txt");
        return fnm;
	} else if (m_EetType == ELT_LAVA){
        static DECLARE_CTFILENAME(fnm, "Data\\Messages\\Enemies\\ElementalLava.txt");
        return fnm;
	} else if (m_EetType == ELT_STONE){
        static DECLARE_CTFILENAME(fnm, "Data\\Messages\\Enemies\\ElementalStone.txt");
        return fnm;
	} else if (m_EetType ==  ELT_WATER){
        static DECLARE_CTFILENAME(fnm, "Data\\Messages\\Enemies\\ElementalWater.txt");
        return fnm;
    }
    ...
    case ELT_LAVA:
		{
			...
			{
				PrecacheClass(CLASS_PROJECTILE, PRT_LAVAMAN_LARGE_FIRE);
			}
			if( (m_EecChar == ELC_LARGE) || (m_EecChar == ELC_BIG) )
			{
				PrecacheClass(CLASS_PROJECTILE, PRT_LAVAMAN_BIG_FIRE);
			}

			PrecacheClass(CLASS_PROJECTILE, PRT_LAVAMAN_STONE);
			PrecacheClass(CLASS_PROJECTILE, PRT_LAVAMAN_FIRE);

			PrecacheModel  (MODEL_LAVA            );
			PrecacheModel  (MODEL_LAVA_BODY_FLARE );
			PrecacheModel  (MODEL_LAVA_HAND_FLARE );
			PrecacheModel  (MODEL_ELEM_LAVASTONE );
			PrecacheModel ( MODEL_ELEM_LAVASTONE_FLARE);
			PrecacheTexture(TEXTURE_LAVA          );
			PrecacheTexture(TEXTURE_LAVA_DETAIL   );
			PrecacheTexture(TEXTURE_LAVA_FLARE    );
			PrecacheTexture(TEXTURE_ELEM_LAVASTONE    );
			PrecacheTexture(TEXTURE_AIR_F    );
	...
	case ELT_AIR:
		{
			...
			PrecacheModel  (MODEL_AIR_TWISTER    );
			PrecacheTexture(TEXTURE_AIR          );
    ...
    case ELT_ICE:
		{
			...
			PrecacheSound(SOUND_LAVA_ANGER   );
			PrecacheSound(SOUND_LAVA_GROW    );	
			PrecacheTexture(TEXTURE_ELEM_FLARE    );
			break;
		}
    ...
    case ELT_STONE:
		{
			if( m_EecChar == ELC_LARGE)
			{
				PrecacheClass(CLASS_PROJECTILE, PRT_STONEMAN_LARGE_FIRE);
			}
			if( (m_EecChar == ELC_LARGE) || (m_EecChar == ELC_BIG) )
			{
				PrecacheClass(CLASS_PROJECTILE, PRT_STONEMAN_BIG_FIRE);
			}
      ...
      case ELT_WATER:
		{
			...
			PrecacheModel  (MODEL_WATER_BODY_FLARE );
			PrecacheModel  (MODEL_WATER            );
			PrecacheModel  (MODEL_WATER_DROP_PROJECTIVE);
			PrecacheTexture(TEXTURE_WATER          );
  ...
  INDEX AnimForDamage(FLOAT fDamage) {
    ...
    if (m_EetType == ELT_LAVA) {
      switch (IRnd()%3) {
        case 0: iAnim = LAVAMAN_ANIM_WOUND01; break;
        case 1: iAnim = LAVAMAN_ANIM_WOUND02; break;
        default: iAnim = LAVAMAN_ANIM_WOUND03; break;
      }
    } else {
      switch (IRnd()%3) {
  ...
  void StandingAnimFight(void) {
    StartModelAnim(LAVAMAN_ANIM_ATTACK06, AOF_LOOPING|AOF_NORESTART);
  }
  ...
  void StandingAnim(void) {
    if (m_EetType == ELT_LAVA) {
      switch (m_EesCurrentState) {
        case ELS_NORMAL: StartModelAnim(LAVAMAN_ANIM_WALK, AOF_LOOPING|AOF_NORESTART); break;
        case ELS_BOX: StartModelAnim(LAVAMAN_ANIM_STANDBOX, AOF_LOOPING|AOF_NORESTART); break;
        case ELS_PLANE:
        default: StartModelAnim(LAVAMAN_ANIM_STANDPLANE, AOF_LOOPING|AOF_NORESTART); break;
      }
    } else {
      switch (m_EesCurrentState) {
  ...
  void WalkingAnim(void)
  {
    if (m_EetType == ELT_LAVA) {
      if (m_EecChar==ELC_LARGE) {
        StartModelAnim(LAVAMAN_ANIM_WALK, AOF_LOOPING|AOF_NORESTART);
  ...
  void SpawnNewElemental(void) 
  {
	...
	switch (m_EetType) {
      case ELT_AIR:
        eSpawnSpray.sptType = SPT_AIRICESPRAY;
        break;
      case ELT_ICE:
        eSpawnSpray.sptType = SPT_AIRICESPRAY;
        break;
      ...
      case ELT_STONE:
        ...
        break;
      case ELT_WATER:
       eSpawnSpray.sptType = SPT_WATERSPRAY;
        break;
    }
  ...
  void ThrowRocks(ProjectileType EptProjectile) {
    ...
    // throw rocks, exclusive for lavaman
  void ThrowLavaRocks(ProjectileType EptProjectile) {
    // projectile type and position
    FLOAT3D vPos;
    ANGLE3D aAngle;
    // throw rocks
    switch (m_EecChar) {
      ...
      case ELC_BIG: {
        vPos = FIRE_ROCKS_BIG;
        ShootProjectile(EptProjectile, FLOAT3D(0.5f,8.5f,0), ANGLE3D(0, 0, 0));
        aAngle = ANGLE3D(FRnd()*4.0f+4.0f, FRnd()*3.0f-2.0f, 0);
        ShootProjectile(EptProjectile, FLOAT3D(0.5f,8.5f,0), aAngle);
        aAngle = ANGLE3D(FRnd()*-4.0f-4.0f, FRnd()*3.0f-2.0f, 0);
        ShootProjectile(EptProjectile, FLOAT3D(0.5f,8.5f,0), aAngle);
        break;}
 ...
 ************************************************************/
 FallOnFloor(EVoid) {
    ...
    StartModelAnim(LAVAMAN_ANIM_MORPHPLANEUP, 0);
    return EReturn();
  };
 ...
 ************************************************************/
  ...
  //
  LavamanFire(EVoid) {
      StartModelAnim(LAVAMAN_ANIM_ATTACK06, 0);
    // throw rocks
    if (m_EecChar==ELC_LARGE) {
	    autowait(0.95f);
      ThrowLavaRocks(PRT_LAVAMAN_LARGE_FIRE);
    } else if (m_EecChar==ELC_BIG) {
	    autowait(0.95f);
      ThrowLavaRocks(PRT_LAVAMAN_BIG_FIRE);
    } else if  (m_EecChar==ELC_SMALL) {
	  	StartModelAnim(LAVAMAN_ANIM_ATTACK05, 0);
		autowait(0.7f);
	  ShootProjectile(PRT_LAVAMAN_FIRE, LAVAMAN_FIRE_SMALL, ANGLE3D(0, 0, 0));
    }
    PlaySound(m_soSound, SOUND_LAVA_FIRE, SOF_3D);
    autowait(0.9f);
    // stand a while
  ...
  LavamanHit(EVoid) {
    StartModelAnim(LAVAMAN_ANIM_ATTACK02, 0);
    autowait(0.6f);
    HitGround();
    PlaySound(m_soSound, SOUND_LAVA_KICK, SOF_3D);
    autowait(0.5f);
  ...
  /*LavamanFire(EVoid)
  {
    ...
    if (m_EecChar==ELC_LARGE)
    {
      ...
      StartModelAnim(LAVAMAN_ANIM_ATTACK03, AOF_SMOOTHCHANGE);
      ...
      StartModelAnim(LAVAMAN_ANIM_WALK, AOF_SMOOTHCHANGE);
    ...
    else if (m_EecChar==ELC_BIG)
    {
      CModelObject &mo = *GetModelObject();
      FLOAT tmWait = mo.GetAnimLength( mo.ao_iCurrentAnim )-mo.GetPassedTime();
      StartModelAnim(LAVAMAN_ANIM_ATTACK04, AOF_SMOOTHCHANGE);
    ...
    autowait( GetModelObject()->GetAnimLength( LAVAMAN_ANIM_ATTACK04) - 0.9f);
  ...
  LavamanHit(EVoid)
  {
    StartModelAnim(LAVAMAN_ANIM_ATTACK01, 0);
    autowait(0.6f);
    HitGround();
 ...
 ************************************************************/
  ...
  PlaneToNormal(EVoid) {
    ...
    INDEX iAnim;
    if (m_EetType == ELT_LAVA) {
      iAnim = LAVAMAN_ANIM_MORPHPLANEUP;
    } else {
 ...
 ************************************************************/
  ...
  BossAppear(EVoid)
  {
    GetModelObject()->PlayAnim(LAVAMAN_ANIM_MORPHPLANEUP, 0);
  ...
  Main(EVoid) {
	...
	switch (m_EetType) {
      ...
      case ELT_STONE:
        ...
        break;
      case ELT_WATER:
       m_sptType = SPT_WATERSPRAY;
        break;
    }
	
	// spawn debris only for small air, ice and stone types
	switch (m_EetType) {
		case ELT_ICE: case ELT_STONE: case ELT_AIR: case ELT_LAVA:
		  if (m_EecChar!=ELC_SMALL) {
