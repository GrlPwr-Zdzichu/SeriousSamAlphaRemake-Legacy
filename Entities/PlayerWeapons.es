...
#include "Models/Weapons/Cannon/Fuse.h"
#include "Models/Weapons/LRG/LRG.h"
#include "Models/Weapons/LRG/Barrel.h"
...
void CPlayerWeapons_Precache(ULONG ulAvailable)
{
  ...
  if ( ulAvailable&(1<<(WEAPON_GRENADELAUNCHER-1)) ) {
    ...
    pdec->PrecacheModel(MODEL_GL_MAIN_PART          ); 
    pdec->PrecacheModel(MODEL_GL_SECOND_PART        ); 
    pdec->PrecacheModel(MODEL_GL_THRID_PART         ); 
	pdec->PrecacheModel(MODEL_GL_LAST_PART          ); 
  ...
  if ( ulAvailable&(1<<(WEAPON_PIPEBOMB-1)) ) {
	...
	pdec->PrecacheModel(MODEL_PIPEBOMB_STICKHAND    );
  ...
  if ( ulAvailable&(1<<(WEAPON_IRONCANNON-1)) /*||
	...
	pdec->PrecacheModel(MODEL_CN_FUSE   );
	pdec->PrecacheTexture(TEXTURE_FUSE);
    pdec->PrecacheSound(SOUND_CANNON    );
    pdec->PrecacheSound(SOUND_CANNON_PREPARE);
    pdec->PrecacheClass(CLASS_CANNONBALL);
  }

if ( ulAvailable&(1<<(WEAPON_LRG-1)) ) {
    pdec->PrecacheModel(MODEL_LRG     );
    pdec->PrecacheModel(MODEL_LR_BODY         );
    pdec->PrecacheModel(MODEL_LR_BARREL      );
    pdec->PrecacheTexture(TEXTURE_BODY  );  
    pdec->PrecacheTexture(TEXTURE_BARREL     );      
    pdec->PrecacheSound(SOUND_LR_FIRE         );
	pdec->PrecacheClass(CLASS_PROJECTILE, PRT_LAVASTONE);
  }
...
static FLOAT afMinigunPos[4] = { 0.0f, -1.075f, 0.0f};
...
static FLOAT afTommygunShellPos[3] = { 0.26f, -0.3f, -0.31f};
...
// 55 INDEX m_iMaxNukeBalls   = MAX_NUKEBALLS,
 54 INDEX m_iRocks			= 0,
 55 INDEX m_iMaxRocks		= MAX_ROCKS,
...
101 model   MODEL_GL_MAIN_PART          "Models\\Weapons\\GrenadeLauncher\\MainPart.mdl",
102 model   MODEL_GL_SECOND_PART        "Models\\Weapons\\GrenadeLauncher\\SecondPart.mdl",
103 model   MODEL_GL_THRID_PART         "Models\\Weapons\\GrenadeLauncher\\ThridPart.mdl",
104 model   MODEL_GL_LAST_PART          "Models\\Weapons\\GrenadeLauncher\\LastPart.mdl",
105 texture TEXTURE_GL_BODY             "Models\\Weapons\\GrenadeLauncher\\Body.tex",
...
121 model   MODEL_PIPEBOMB_STICKHAND    "Models\\Weapons\\Pipebomb\\HandWithStick.mdl",
...
174 model   MODEL_CN_FUSE               "Models\\Weapons\\Cannon\\Fuse.mdl",
175 texture TEXTURE_FUSE              "Models\\Weapons\\Cannon\\Fuse.tex",
176 sound   SOUND_CANNON                "Models\\Weapons\\Cannon\\Sounds\\Fire.wav",
177 sound   SOUND_CANNON_PREPARE        "Models\\Weapons\\Cannon\\Sounds\\Prepare.wav",

// ************** LRG **************
180 model   MODEL_LRG                   "Models\\Weapons\\LRG\\LRG.mdl",
181 model   MODEL_LR_BODY               "Models\\Weapons\\LRG\\Body.mdl",
182 model   MODEL_LR_BARREL             "Models\\Weapons\\LRG\\Barrel.mdl",
183 texture TEXTURE_BODY                "Models\\Weapons\\LRG\\Body.tex",
184 texture TEXTURE_BARREL              "Models\\Weapons\\LRG\\Barrel.tex",
185 sound   SOUND_LR_FIRE                "Models\\Weapons\\LRG\\Sounds\\Fire.wav",
...
//    if( iWeaponData==WEAPON_NUKECANNON) { iWeaponData = WEAPON_IRONCANNON; }
  ...
  void UpdateTargetingInfo(void)
  {
    ...
    FLOAT fFY = wpn_fFY[m_iCurrentWeapon] + 0.11f;
...
/*    if (!IsPredictionHead()) {
  ...
  void SetCurrentWeaponModel(void) {
    ...
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_KNIFE:
        ...
        m_moWeapon.PlayAnim(KNIFE_ANIM_STAND1B, 0);
		m_moWeapon.PlayAnim(KNIFE_ANIM_STAND3B, 0);
		m_moWeapon.PlayAnim(KNIFE_ANIM_STAND2B, 0);
		m_moWeapon.StretchModel(FLOAT3D(0.6f,0.6f,0.6f));
        break;
      // colt
      ...
      case WEAPON_SINGLESHOTGUN: {
		...
		m_moWeapon.StretchModel(FLOAT3D(0.75f,0.75f,0.75f));
        break; }
      case WEAPON_DOUBLESHOTGUN: {
      ...
      case WEAPON_TOMMYGUN: {
		...
		//Nad tym trza pomyslec
		m_moWeapon.StretchModel(FLOAT3D(0.845f,0.845f,0.845f/*,0.87*/));
        break; }
      case WEAPON_MINIGUN: {
      ...
      case WEAPON_GRENADELAUNCHER:
        ...
        AddAttachmentToModel(this, m_moWeapon, GRENADELAUNCHER_ATTACHMENT_MAIN_PART, MODEL_GL_MAIN_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);                   
        AddAttachmentToModel(this, m_moWeapon, GRENADELAUNCHER_ATTACHMENT_SECOND_PART, MODEL_GL_SECOND_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddAttachmentToModel(this, m_moWeapon, GRENADELAUNCHER_ATTACHMENT_THIRD_PART, MODEL_GL_THRID_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);  
		AddAttachmentToModel(this, m_moWeapon, GRENADELAUNCHER_ATTACHMENT_LAST_PART, MODEL_GL_LAST_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
		m_moWeapon.StretchModel(FLOAT3D(1,1,1));
        break;
      ...
      case WEAPON_PIPEBOMB:
        ...
        SetComponents(this, m_moWeaponSecond, MODEL_PIPEBOMB_STICKHAND, TEXTURE_HAND, 0, 0, 0);
      ...
      case WEAPON_IRONCANNON:
		...
		AddAttachmentToModel(this, m_moWeapon, CANNON_ATTACHMENT_FUSE, MODEL_CN_FUSE, TEXTURE_FUSE, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
		m_moWeapon.StretchModel(FLOAT3D(1,1,1));
        break;

	  case WEAPON_LRG:
        SetComponents(this, m_moWeapon, MODEL_LRG, TEXTURE_HAND, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeapon, LRG_ATTACHMENT_BODY, MODEL_LR_BODY, TEXTURE_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddAttachmentToModel(this, m_moWeapon, LRG_ATTACHMENT_BARREL, MODEL_LR_BARREL, TEXTURE_BARREL, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
		m_moWeapon.StretchModel(FLOAT3D(0.61f,0.61f,0.61f));
        break;
    }
  };
  ...
  void FireGrenade(INDEX iPower) {
    ...
    eLaunch.fSpeed = 35.0f+iPower*35.0f;
...
// fire Lava Rocks
  void FireLavaRocks(void) {
    // Lava Rocks Gun start position
    CPlacement3D plLavaStone;
    FLOAT fFX = wpn_fFX[WEAPON_LRG];  // get LRG's center position
    FLOAT fFY = wpn_fFY[WEAPON_LRG];
    FLOAT fLUX = 0.1f;
    FLOAT fLUY = -0.1f;
    switch(m_iLavaBarrel) {
      case 0:   // barrel pos
        CalcWeaponPosition(FLOAT3D(fFX+fLUX, fFY+fLUY, 0), plLavaStone, TRUE);
        break;
    }
    // create laser projectile
    CEntityPointer penLRG = CreateEntity(plLavaStone, CLASS_PROJECTILE);
    // init and launch laser projectile
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.prtType = PRT_LAVASTONE;
    penLRG->Initialize(eLaunch);
  };
...
//    m_iMaxNukeBalls   = ClampUp((INDEX) ceil(MAX_NUKEBALLS*fModifier),    INDEX(999));
	...
	m_iMaxRocks		  = ClampUp((INDEX) ceil(MAX_ROCKS*fModifier),        INDEX(999));
...
//    if( iTakeAmmo & (1<<AMMO_NUKEBALLS))     {m_iNukeBalls  = 0;}
	...
	if( iTakeAmmo & (1<<AMMO_ROCKS))		 {m_iRocks  = 0;}
...
//    case WEAPON_NUKECANNON:      return m_iNukeBalls;
      case WEAPON_LRG:			   return m_iRocks;
    }
    return 0;
  };
  ...
  INDEX GetMaxAmmo(void)
  {
    switch (m_iCurrentWeapon) {
	  ...
	  case WEAPON_LRG:			   return m_iMaxRocks;
...
//    m_iNukeBalls   = ClampUp(m_iNukeBalls,    m_iMaxNukeBalls);
    m_iRocks	   = ClampUp(m_iRocks,		  m_iMaxRocks);
  }
...
/*      // for nuke ball
      ...
      case WEAPON_FLAMER:
        ...
        AddManaToPlayer( iAmmoPicked*300.0f*MANA_AMMO);
        break;
	  case WEAPON_LRG:
        // for Rocks
        iAmmoPicked = Max(50.0f, m_iMaxRocks*fMaxAmmoRatio);
        m_iRocks += iAmmoPicked;
        AddManaToPlayer( iAmmoPicked*700.0f*MANA_AMMO);
        break;
      // error
      default:
  ...
  void DropWeapon(void) 
  {
    ...
    switch (m_iCurrentWeapon) {
	  ...
	  case WEAPON_LRG : wit = WIT_LRG; break;
    }
  ...
  BOOL ReceiveWeapon(const CEntityEvent &ee) {
    ...
    switch (Ewi.iWeapon) {
	  ...
	  case WIT_LRG: Ewi.iWeapon = WEAPON_LRG; break;
      default:
        ASSERTALWAYS("Uknown weapon type");
    }
...
/*
    ...
    */
    ...
    switch (wit) {
      case WIT_COLT:            
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Colt"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\colt.txt"); 
        break;
      case WIT_SINGLESHOTGUN:   
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Single Shotgun"), 0);
      ...
      case WIT_DOUBLESHOTGUN:   
        ...
        break;
      case WIT_TOMMYGUN:        
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Tommygun"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\tommygun.txt"); 
        break;
      case WIT_MINIGUN:         
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Minigun"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\minigun.txt"); 
        break;
      case WIT_ROCKETLAUNCHER:  
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Rocket Launcher"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\rocketlauncher.txt"); 
        break;
      case WIT_GRENADELAUNCHER: 
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Grenade Launcher"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\grenadelauncher.txt"); 
        break;
      case WIT_PIPEBOMB:       
		((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Pipebomb"), 0); 
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\pipebomb.txt"); 
        break;
      case WIT_FLAMER: 
	  ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Flamethrower"), 0);         
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\flamer.txt"); 
        break;
      case WIT_LASER:           
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Lasergun"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\laser.txt"); 
        break;
      case WIT_GHOSTBUSTER:   
		((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Ghost Buster"), 0);  
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\ghostbuster.txt"); 
        break;
      case WIT_CANNON:          
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Cannon"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\cannon.txt"); 
        break;
      case WIT_LRG:          
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Lava Rocks Gun"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\lrg.txt"); 
        break;
      default:
        ASSERTALWAYS("Uknown weapon type");
    }
  ...
  BOOL ReceiveAmmo(const CEntityEvent &ee) {
    ...
    switch (Eai.EaitType) {
      ...
      case AIT_ELECTRICITY:
        ...
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Batteries"), Eai.iQuantity);
...
/*      // cannon balls
      ...
      case AIT_NAPALM:
        ...
        break;
     case AIT_ROCKS:
        if (m_iRocks>=m_iMaxRocks) { m_iRocks = m_iMaxRocks; return FALSE; }
        m_iRocks+= Eai.iQuantity;
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Lava rocks"), Eai.iQuantity);
        AddManaToPlayer(Eai.iQuantity*AV_ROCKS *MANA_AMMO);
        break;
      case AIT_BACKPACK:
      ...
      case AIT_SERIOUSPACK:
		...
		m_iNapalm += MAX_NAPALM*GetSP()->sp_fAmmoQuantity;
		m_iRocks += MAX_ROCKS*GetSP()->sp_fAmmoQuantity;
//        m_iNukeBalls += MAX_NUKEBALLS*GetSP()->sp_fAmmoQuantity;
  ...
  BOOL ReceivePackAmmo(const CEntityEvent &ee)
  {
    ...
    if( (eapi.iShells>0 && m_iShells<m_iMaxShells) ||
		...
		(eapi.iRocks>0 && m_iRocks<m_iMaxRocks)
...
//      m_iNukeBalls+=eapi.iNukeBalls;
      ...
      if( eapi.iElectricity != 0) {strMessage.PrintF("%s %d %s,", strMessage, eapi.iElectricity, TRANS("Batteries"));}
      if( eapi.iIronBalls != 0) {strMessage.PrintF("%s %d %s,", strMessage, eapi.iIronBalls, TRANS("Cannonballs"));}
	  if( eapi.iRocks != 0) {strMessage.PrintF("%s %d %s,", strMessage, eapi.iRocks, TRANS("Lava rocks"));}
//      if( eapi.iNukeBalls != 0) {strMessage.PrintF("%s %d %s,", strMessage, eapi.iNukeBalls, TRANS("Nuke balls"));}
  ...
  WeaponType GetStrongerWeapon(INDEX iWeapon) {
    switch(iWeapon) {
      ...
      case 9: return WEAPON_LASER;
      case 10: return WEAPON_IRONCANNON;
	  case 11: return WEAPON_LRG;
    }
    return WEAPON_NONE;
  };
  ...
  INDEX GetSelectedWeapon(WeaponType EwtSelectedWeapon) {
    switch(EwtSelectedWeapon) {
      ...
      case WEAPON_PIPEBOMB: case WEAPON_GRENADELAUNCHER: return 6;
      case WEAPON_FLAMER: return 7;
      case WEAPON_GHOSTBUSTER: case WEAPON_LASER: return 8;
      case WEAPON_IRONCANNON: /*case WEAPON_NUKECANNON: */return 10;
	  case WEAPON_LRG: return 11;
    }
    return 0;
  };
  ...
  WeaponType GetAltWeapon(WeaponType EwtWeapon) {
    switch (EwtWeapon) {
      ...
      case WEAPON_GRENADELAUNCHER: return WEAPON_PIPEBOMB;
	  case WEAPON_PIPEBOMB: return WEAPON_GRENADELAUNCHER;
...
//      case WEAPON_NUKECANNON: return WEAPON_IRONCANNON;
      case WEAPON_LRG: return WEAPON_LRG;
    }
    return WEAPON_NONE;
  };
...
/*
      ...
      case WEAPON_PIPEBOMB:
        ...
        WeaponSelectOk(WEAPON_KNIFE);
        break;
      case WEAPON_LASER:  case WEAPON_FLAMER: case WEAPON_GHOSTBUSTER: case WEAPON_LRG:
        WeaponSelectOk(WEAPON_GHOSTBUSTER)||
        WeaponSelectOk(WEAPON_LASER)||
        WeaponSelectOk(WEAPON_FLAMER)||
		WeaponSelectOk(WEAPON_LRG)||
        WeaponSelectOk(WEAPON_MINIGUN)||
        WeaponSelectOk(WEAPON_TOMMYGUN)||
        WeaponSelectOk(WEAPON_DOUBLESHOTGUN)||
        WeaponSelectOk(WEAPON_SINGLESHOTGUN)||
...
//      case WEAPON_NUKECANNON: return (m_iNukeBalls>0);
      case WEAPON_LRG: return (m_iRocks>0);
    }
    return FALSE;
  };
    ...
    switch(m_iCurrentWeapon) {
      ...
      case WEAPON_KNIFE:
        switch (m_iKnifeStand) {
          case 1: m_moWeapon.PlayAnim(KNIFE_ANIM_STAND1B, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
          case 3: m_moWeapon.PlayAnim(KNIFE_ANIM_STAND3B, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
	  case 2: m_moWeapon.PlayAnim(KNIFE_ANIM_STAND2B, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
      ...
      case WEAPON_IRONCANNON:
//      case WEAPON_NUKECANNON:
        m_moWeapon.PlayAnim(CANNON_ANIM_WAIT1, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
		case WEAPON_LRG:
        m_moWeapon.PlayAnim(LRG_ANIM_WAIT01, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
      default:
...
//   *  >>>---   WEAPON BORING   ---<<<
  ...
  FLOAT KnifeBoring(void) {
    ...
    switch (m_iKnifeStand) {
	case 1:
     iAnim = KNIFE_ANIM_STAND1A;
	 break;
    case 3:
	   iAnim = KNIFE_ANIM_STAND3A; 
	   break;
     case 2:
	iAnim = KNIFE_ANIM_STAND2A;
	break;
	 iAnim = KNIFE_ANIM_STAND2C;
	 break;
  ...
  FLOAT CannonBoring(void) {
    ...
    INDEX iAnim;
    switch (IRnd()%3) {
      case 0: iAnim = CANNON_ANIM_WAIT2; break;
      case 1: iAnim = CANNON_ANIM_WAIT3; break;
      case 2: iAnim = CANNON_ANIM_WAIT4; break;
    }
    m_moWeapon.PlayAnim(iAnim, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(iAnim);
  };

  FLOAT LavaRocksBoring(void) {
    // play boring anim
    INDEX iAnim;
    iAnim = LRG_ANIM_WAIT01;
    m_moWeapon.PlayAnim(iAnim, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(iAnim);
  };
  ...
  PutDown() {
    ...
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_KNIFE: 
        ...
        } else if (m_iKnifeStand==1) {
          m_iAnim = KNIFE_ANIM_PULLOUT3;
		} else if (m_iKnifeStand==2) {
          m_iAnim = KNIFE_ANIM_PULLOUT2;
...
/*
      ...
      wait() {
        on (EBegin) : {
          ...
          switch (m_iCurrentWeapon) {
			...
			case WEAPON_LRG: call FireLavaRocks(); break;
            default: ASSERTALWAYS("Unknown weapon.");
          }
          resume;
        }
  ...
  SwingKnife() {
    ...
    switch (m_iKnifeStand) {
      case 1:
        iSwing = IRnd()%2;
        switch (iSwing) {
          case 0: m_iAnim = KNIFE_ANIM_STAND1ATTACK01; m_fAnimWaitTime = 0.25f;
            ...
            break;
          case 1: m_iAnim = KNIFE_ANIM_STAND1ATTACK03; m_fAnimWaitTime = 0.35f;
            PlaySound(pl.m_soWeapon1, SOUND_KNIFE_LOW, SOF_3D|SOF_VOLUMETRIC);
            if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("Knife_low");}
            break;
        }
        break;
      case 3:
        iSwing = IRnd()%2;
        switch (iSwing) {
          case 0: m_iAnim = KNIFE_ANIM_STAND3ATTACK01; m_fAnimWaitTime = 0.50f;
            PlaySound(pl.m_soWeapon1, SOUND_KNIFE_LOW, SOF_3D|SOF_VOLUMETRIC);
            if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("Knife_low");}
            break;
          case 1: m_iAnim = KNIFE_ANIM_STAND3ATTACK02; m_fAnimWaitTime = 0.50f;
            PlaySound(pl.m_soWeapon3, SOUND_KNIFE_LONG, SOF_3D|SOF_VOLUMETRIC);
            if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("Knife_long");}
            break;
        }
        break;
	  case 2:
        iSwing = IRnd()%3;
        switch (iSwing) {
          case 0: m_iAnim = KNIFE_ANIM_STAND2ATTACK01; m_fAnimWaitTime = 0.50f;
            PlaySound(pl.m_soWeapon1, SOUND_KNIFE_BACK, SOF_3D|SOF_VOLUMETRIC);
            if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("Knife_back");}
            break;
          case 1: m_iAnim = KNIFE_ANIM_STAND2ATTACK02; m_fAnimWaitTime = 0.50f;
            PlaySound(pl.m_soWeapon3, SOUND_KNIFE_HIGH, SOF_3D|SOF_VOLUMETRIC);
            if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("Knife_high");}
            break;
		  case 2: m_iAnim = KNIFE_ANIM_STAND2ATTACK03; m_fAnimWaitTime = 0.50f;
    ...
    m_moWeapon.PlayAnim(m_iAnim, 0);
	  if (m_iKnifeStand==1) {
	  autowait(0.2f);
	  CutWithKnife(0, 0, /*range=*/3.0f, /*wide=*/2.0f, /*thickness=*/0.5f, /*damage=*/45.0f);
	  autowait(m_fAnimWaitTime);
	  } else if (m_iKnifeStand==3) {
	  autowait(0.5f);
	  CutWithKnife(0, 0, /*range=*/3.0f, /*wide=*/2.0f, /*thickness=*/0.5f, /*damage=*/215.0f);
	  autowait(m_fAnimWaitTime);
	  } else  if (m_iKnifeStand==2) {
	  autowait(0.15f);
	  CutWithKnife(0, 0, /*range=*/3.0f, /*wide=*/2.0f, /*thickness=*/0.5f, /*damage=*/75.0f);
	  autowait(m_fAnimWaitTime);
    };
  ...
  FireSingleShotgun() {
    ...
    INDEX iAnim;
    switch (IRnd()%2) {
      case 0: iAnim = SINGLESHOTGUN_ANIM_FIRE1; break;
      case 1: iAnim = SINGLESHOTGUN_ANIM_FIRE2; break;
    }
	m_moWeapon.PlayAnim(iAnim, 0);
      //m_moWeapon.PlayAnim(GetSP()->sp_bCooperative ? SINGLESHOTGUN_ANIM_FIRE1 : SINGLESHOTGUN_ANIM_FIRE1, 0);
      // sound
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      ...
      /* drop shell */
        autowait(m_moWeapon.GetAnimLength(
          (GetSP()->sp_bCooperative ? SINGLESHOTGUN_ANIM_FIRE1:SINGLESHOTGUN_ANIM_FIRE1) ) -
  ...
  FireDoubleShotgun() {
    ...
    if (m_iShells>1) {
      ...
      SetFlare(0, FLARE_ADD);
      PlayLightAnim(LIGHT_ANIM_COLT_SHOTGUN, 0);
      m_moWeapon.PlayAnim(GetSP()->sp_bCooperative ? DOUBLESHOTGUN_ANIM_FIRE : DOUBLESHOTGUN_ANIM_FIRE, 0);
      m_moWeaponSecond.PlayAnim(GetSP()->sp_bCooperative ? HANDWITHAMMO_ANIM_FIRE : HANDWITHAMMO_ANIM_FIRE, 0);
      // sound
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      ...
      autowait( m_moWeapon.GetAnimLength(
        (GetSP()->sp_bCooperative ? DOUBLESHOTGUN_ANIM_FIRE : DOUBLESHOTGUN_ANIM_FIRE)) -
  ...
  FireGrenadeLauncher()
  {
    ...
    if (m_iGrenades>0)
    {
     // sound
      ...
      m_moWeapon.PlayAnim(GRENADELAUNCHER_ANIM_FIRE, 0);
	  //fire grenade
...
// ***************** FIRE LAVA ROCKS *****************
    ...
    // fire one rock
    if (m_iRocks>0) {;
		  m_fAnimWaitTime = 0.9f;	  
      m_moWeapon.PlayAnim(LRG_ANIM_FIRE, AOF_LOOPING|AOF_NORESTART);
      FireLavaRocks();
	  autowait(0.25f);
      DecAmmo(m_iRocks, 1);
      // sound
      ...
      switch(m_iLavaBarrel) {
        case 0: {
          CModelObject *pmo = &(m_moWeapon.GetAttachmentModel(LRG_ATTACHMENT_BARREL)->amo_moModelObject);
		  PlaySound(pl.m_soWeapon0, SOUND_LR_FIRE, SOF_3D|SOF_VOLUMETRIC);
          break; }
      }
      // no Rocks -> change weapon
      if (m_iRocks<=0) { SelectNewWeapon(); }
    } else {
      ASSERTALWAYS("LRG - Auto weapon change not working.");
      m_bFireWeapon = m_bHasAmmo = FALSE;
    }
    return EEnd();
  };
  ...
  ChangeKnifeStand(EVoid) {
    if (m_iKnifeStand==1) {
      ...
      // change from knife stand 3 to stand 2
      m_moWeapon.PlayAnim(KNIFE_ANIM_STAND3TOSTAND2, 0);
      autowait(m_moWeapon.GetAnimLength(KNIFE_ANIM_STAND3TOSTAND2));
      m_iKnifeStand = 2;
	} else if (m_iKnifeStand==2) {
      // change from knife stand 2 to stand 1
      m_moWeapon.PlayAnim(KNIFE_ANIM_STAND2TOSTAND3, 0);
      autowait(m_moWeapon.GetAnimLength(KNIFE_ANIM_STAND2TOSTAND3));
      m_iKnifeStand = 3;
    }
    return EEnd();
  };
...
*/
  ...
  BoringWeaponAnimation() {
    ...
    switch (m_iCurrentWeapon) {
	  ...
	  case WEAPON_LRG: fWait = LavaRocksBoring(); break;
      default: ASSERTALWAYS("Unknown weapon.");
    }
