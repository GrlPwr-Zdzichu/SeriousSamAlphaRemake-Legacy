...
// 16/9 Fix
extern INDEX ssa_bUseResolutionFix;
extern INDEX ssa_bShowWeaponChange;
...
static CTextureObject _toARocks;
// weapon textures
...
static struct AmmoInfo _aaiAmmo[8] = {
  ...
  { &_toARocks,		  &_awiWeapons[17], NULL,             0, 0, 0, -9, FALSE },
};

struct WeaponInfo _awiWeapons[18] = {
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   //  0
  { WEAPON_KNIFE,           &_toWKnife,           NULL,         FALSE },   //  1
  { WEAPON_COLT,            &_toWColt,            &_aaiAmmo[1],         FALSE },   //  2
  { WEAPON_DOUBLECOLT,      &_toWColt,            &_aaiAmmo[1],         FALSE },   //  3
  ...
  { WEAPON_PIPEBOMB,        &_toWPipeBomb,        &_aaiAmmo[3], FALSE },   // 10
  ...
  { WEAPON_GHOSTBUSTER,     &_toWGhostBuster,     &_aaiAmmo[5], FALSE },   // 15
  { WEAPON_IRONCANNON,      &_toWIronCannon,      &_aaiAmmo[6], FALSE },   // 16
  { WEAPON_LRG,			    &_toWLRG,			  &_aaiAmmo[7], FALSE },   // 17
};
...
static void FillWeaponAmmoTables(void)
{
  ...
  _aaiAmmo[7].ai_iAmmoAmmount    = _penWeapons->m_iRocks;
  _aaiAmmo[7].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxRocks;
...
extern void DrawHUD( const CPlayer *penPlayerCurrent, CDrawPort *pdpCurrent, BOOL bSnooping)
{
  ...
  COLOR colMax = C_CYAN;
  COLOR colTop = C_GREEN;
  COLOR colMed = C_YELLOW;
  COLOR colMid = LerpColor(colMed, C_RED, 0.5f);

  // adjust borders color in case of spying mode
  COLOR colBorder =  0x46464600; 
  if( bSnooping) {
    UBYTE ubR,ubG,ubB;
  ...
  if( fValue > 0.0f) {
    ...
    PrepareColorTransitions( colMax, colTop, colMed, C_lGRAY, 0.4f, 0.25f, TRUE);
    ...
    fCol += fAdvUnit+fChrUnit*1.7/2.15 -fHalfUnit;
    //HUD_DrawBorder( fCol, fRow, fChrUnit*3, fOneUnit, colBorder);
	if (ssa_bUseResolutionFix) {
		HUD_DrawText( fCol+3.2, fRow-5.2, strValue, NONE, fNormValue); // 4.5 Stare
	} else {
		HUD_DrawText( fCol+3.2, fRow+3.8, strValue, NONE, fNormValue); // 4.5 Stare
	}
    fCol -= fAdvUnit+fChrUnit*1.9/2 -fHalfUnit;
	if (ssa_bUseResolutionFix) {
		HUD_DrawIcon( fCol-3.25, fRow-6.5, _toArmor, C_WHITE /*_colHUD*/, fNormValue, FALSE);  // 4.5 Stare
	} else {
		HUD_DrawIcon( fCol-3.25, fRow+4, _toArmor, C_WHITE /*_colHUD*/, fNormValue, FALSE);  // 4.5 Stare
	}
  }
  ...
  if( iCurrentWeapon != WEAPON_KNIFE && !GetSP()->sp_bInfiniteAmmo) {
    // determine ammo quantities
    FLOAT fMaxValue = _penWeapons->GetMaxAmmo();
    fValue = _penWeapons->GetAmmo();
    fNormValue = fValue / fMaxValue;
    strValue.PrintF( "%d", (SLONG)ceil(fValue));
    PrepareColorTransitions( colMax, colMax, colMed, C_RED, 0.43f, 0.25f, TRUE);
    BOOL bDrawAmmoIcon = _fCustomScaling<=1.0f;
    // draw ammo, value and weapon
	_fCustomScaling = ClampDn( _fCustomScaling*1.0f, 0.5f);
    fRow = pixBottomBound-fHalfUnit*_pDP->dp_fWideAdjustment;
    fCol = pixLeftBound+    fHalfUnit; fRow;
    //colDefault = AddShaker( 4, fValue, penLast->m_iLastAmmo, penLast->m_tmAmmoChanged, fMoverX, fMoverY);
    //HUD_DrawBorder( fCol+fMoverX, fRow+fMoverY, fOneUnit, fOneUnit, colBorder);
    fCol += fAdvUnit+fChrUnit*46/2  -fHalfUnit;
    //HUD_DrawBorder( fCol, fRow, fChrUnit*3, fOneUnit, colBorder);
    if( bDrawAmmoIcon && ptoCurrentAmmo != NULL) {
      //fCol += fAdvUnit+fChrUnit*-9.0/2.1 - fHalfUnit; // To jest originalne 6/2.1
		// ammo icon
		if (iCurrentWeapon == WEAPON_COLT || iCurrentWeapon == WEAPON_DOUBLECOLT) {
	    fCol = pixRightBound/2 - 1-fAdvUnit;
		} else if (iCurrentWeapon == WEAPON_PIPEBOMB) {
		fCol = pixRightBound/2 - 1-fAdvUnit*1.35; // 59 for hud_fscaling=1
		} else {
		fCol = pixRightBound/2 - 1-fAdvUnit*1.65; // 59 for hud_fscaling=1
		}
		// ammo icon ^
		if (ssa_bUseResolutionFix) {
			HUD_DrawBorder( fCol, fRow, fOneUnit+1400, fOneUnit+4, colBorder);
			HUD_DrawIcon( fCol/*+1*/, fRow+-0.5, *ptoCurrentAmmo, C_WHITE /*_colHUD*/, fNormValue, TRUE);
		} else {
			HUD_DrawBorder( fCol, fRow+4, fOneUnit+1400, fOneUnit+6, colBorder);
			HUD_DrawIcon( fCol/*+1*/, fRow+4, *ptoCurrentAmmo, C_WHITE /*_colHUD*/, fNormValue, TRUE);
		}
      //fCol -= fAdvUnit+fChrUnit*-7.8/2.1 -fHalfUnit;  // To jest originalne 6/2.1
    } //fCol -= fAdvUnit+fChrUnit*1.9/2 -fHalfUnit
	fCol = pixRightBound/2;
	if (ssa_bUseResolutionFix) {
		HUD_DrawText( fCol/*-8.25*/, fRow+0.5, strValue, NONE, fNormValue);
	} else {
		HUD_DrawText( fCol/*-8.25*/, fRow+4, strValue, NONE, fNormValue);
	}
    //fCol -= fAdvUnit+fChrUnit*-4.5/2.1 -fHalfUnit; // To jest originalne 7/2.1
	// Weapon Icon
	if (iCurrentWeapon == WEAPON_COLT || iCurrentWeapon == WEAPON_DOUBLECOLT || iCurrentWeapon == WEAPON_PIPEBOMB) {
	fCol = pixRightBound/2 - 1+fAdvUnit*1.5;
	} else {
	fCol = /*pixRightBound + 39*/pixRightBound/2 - 1+fAdvUnit*2.15; // 72 for hud_fscaling=1
	}
	// Weapon icon ^
	if (ssa_bUseResolutionFix) {
		HUD_DrawIcon( fCol/*+11*/, fRow+-0.5, *ptoCurrentWeapon, C_WHITE /*_colHUD*/, fNormValue, !bDrawAmmoIcon);
	} else {
		HUD_DrawIcon( fCol/*+11*/, fRow+4, *ptoCurrentWeapon, C_WHITE /*_colHUD*/, fNormValue, !bDrawAmmoIcon);  //Stare 5 i 5
	}
  } else if( ptoCurrentWeapon!=NULL) {
    // draw only knife or colt icons (ammo is irrelevant)
    fRow = pixBottomBound-fHalfUnit;
    fCol = +290; fRow; fHalfUnit;
	if (ssa_bUseResolutionFix) {
		HUD_DrawBorder( fCol, fRow, fOneUnit+1000, fOneUnit+4, colBorder);
		HUD_DrawIcon(   fCol+30, fRow+1, *ptoCurrentWeapon, C_WHITE /*_colHUD*/, fNormValue, FALSE);
	} else {
		HUD_DrawBorder( fCol, fRow+4, fOneUnit+1000, fOneUnit+6, colBorder);
		HUD_DrawIcon(   fCol+30, fRow+4, *ptoCurrentWeapon, C_WHITE /*_colHUD*/, fNormValue, FALSE);
	}
  }
  ...
  const INDEX iHealth2 = ClampDn( (INDEX)ceil( _penPlayer->GetHealth()), 0L);
  strHealth.PrintF( "%d", iHealth2);
  fNormValue = fValue/TOP_HEALTH;
  strValue.PrintF( "%d", (SLONG)ceil(fValue));
  PrepareColorTransitions( colMax, colTop, colMed, C_RED, 0.5f, 0.25f, TRUE);
  fRow = pixBottomBound-fHalfUnit;
  fCol = pixLeftBound+fHalfUnit;
  //colDefault = AddShaker( 5, fValue, penLast->m_iLastHealth, penLast->m_tmHealthChanged, fMoverX, fMoverY);
  //HUD_DrawBorder( fCol+fMoverX, fRow+fMoverY, fOneUnit, fOneUnit, colBorder);
  fCol += fAdvUnit+fChrUnit*2/1.8 -fHalfUnit;
  //HUD_DrawBorder( fCol, fRow, fChrUnit*3, fOneUnit, colBorder);
  PrepareColorTransitions( colMax, colTop, colMed, C_RED, 0.4f, 0.25f, TRUE);
  _fdNumbersFont.SetCharSpacing(2.0);
	if (ssa_bUseResolutionFix) {
		HUD_DrawText( fCol, fRow+0.5, strHealth, NONE, fNormValue);
	 } else {
		HUD_DrawText( fCol, fRow+4, strHealth, NONE, fNormValue);
	}


     PrepareColorTransitions( colMax, colTop, colMid, C_GREEN, 0.4f, 0.25f, TRUE); // Stare 5.5
  fCol -= fAdvUnit+fChrUnit*2/1.8 -fHalfUnit;
	if (ssa_bUseResolutionFix) {
		HUD_DrawIcon( fCol-5.4, fRow+0.5, _toHealth, C_WHITE /*_colHUD*/, fNormValue, TRUE);  //Stare 5.5
	} else {
		HUD_DrawIcon( fCol-5.4, fRow+4, _toHealth, C_WHITE /*_colHUD*/, fNormValue, TRUE);  //Stare 5.5
	}
  ...
  _fCustomScaling = ClampDn( _fCustomScaling*0.7f, 0.5f);
  ...
  const FLOAT fBarPos = fHalfUnitS*1.0f;
  ...
  if( (_tmNow - _penWeapons->m_tmWeaponChangeRequired) < hud_tmWeaponsOnScreen) {
    ...
    fRow = pixBottomBound - fHalfUnit - 1.2*fNextUnit;
    fCol = 255.0f - (ctWeapons*fAdvUnit-fHalfUnit)/2.0f;
    // display all available weapons
    for( i=WEAPON_NONE+1; i<WEAPON_LAST; i++) {
      ...
      //HUD_DrawBorder( fCol, fRow, fOneUnit*2, fOneUnit, colIcon);
	  if (ssa_bShowWeaponChange) {
      HUD_DrawIcon(   fCol, fRow, *_awiWeapons[i].wi_ptoWeapon, colIcon, 1.0f, FALSE);
	  }
      // advance to next position
      fCol += fAdvUnit*-1.9 +fOneUnit*4.21;
    }
  }
  ...
  if( _penPlayer->IsConnected() && (_penPlayer->GetFlags()&ENF_ALIVE) && fValue<30.0f) { 
    ...
    PrepareColorTransitions( colMax, colMax, colMed, C_RED, 0.5f, 0.25f, TRUE);
    fNormValue = fValue/30.0f;
    fNormValue = ClampDn(fNormValue, 0.0f);
  //HUD_DrawBorder( fCol,      fRow, fOneUnit,         fOneUnit, colBorder);
  HUD_DrawBorder( fCol+fAdv, fRow, fOneUnit*4*1.9,       fOneUnit, colBorder);
    HUD_DrawBar(    fCol+fAdv, fRow, fOneUnit*4*1.9/*0.975*/, fOneUnit*0.9375, BO_LEFT, NONE, fNormValue);
    HUD_DrawIcon(   fCol*0.9,      fRow*1, _toOxygen, C_WHITE /*_colHUD*/, fNormValue, TRUE);
  ...
  if( _penPlayer->m_penMainMusicHolder!=NULL) {
    ...
    if (fNormValue>0) {
      ...
      HUD_DrawIcon(   fCol,      fRow, _toDeaths, C_WHITE /*_colHUD*/, fNormValue, TRUE);
    }
  }
  ...
  //_fCustomScaling = hud_fScaling;
	  ...
	  if (iScore == 0 && !bFragMatch) {
	  HUD_DrawText(   fCol+fAdv-45, fRow, strValue, C_RED, 1.0f);
	  } else if (iScore < 1000 && !bFragMatch) {
	  HUD_DrawText(   fCol+fAdv-30, fRow, strValue, C_GREEN, 1.0f);
	  } else if (iScore < 10000 && !bFragMatch) {
	  HUD_DrawText(   fCol+fAdv-25, fRow, strValue, C_GREEN, 1.0f);
	  } else if (iScore < 100000 && !bFragMatch) {
	  HUD_DrawText(   fCol+fAdv-20, fRow, strValue, C_GREEN, 1.0f);
	  } else if (iScore < 1000000 && !bFragMatch) {
	  HUD_DrawText(   fCol+fAdv-15, fRow, strValue, C_GREEN, 1.0f);
	  } else if (iScore < 10000000 && !bFragMatch) {
	  HUD_DrawText(   fCol+fAdv-10, fRow, strValue, C_GREEN, 1.0f);
	  } else if (iScore < 100000000 && !bFragMatch) {
	  HUD_DrawText(   fCol+fAdv-5, fRow, strValue, C_GREEN, 1.0f);
	  } else {
		  HUD_DrawText(   fCol+fAdv, fRow, strValue, C_GREEN, 1.0f);
	  }
...
extern void InitHUD(void)
{
  ...
  try {
	...
	_toARocks.SetData_t(    CTFILENAME("Textures\\Interface\\AmRocks.tex"));
	...
	_toWLRG.SetData_t(			   CTFILENAME("Textures\\Interface\\WLRG.tex"));
	...
	((CTextureData*)_toARocks	   .GetData())->Force(TEX_CONSTANT);
	...
	((CTextureData*)_toWLRG		       .GetData())->Force(TEX_CONSTANT);
