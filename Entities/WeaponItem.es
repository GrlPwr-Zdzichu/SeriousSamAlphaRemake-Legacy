...
enum WeaponItemType {
  ...
  8 WIT_PIPEBOMB          "PipeBomb",
  9 WIT_FLAMER            "Flamer",
 10 WIT_LASER             "Laser",
 11 WIT_GHOSTBUSTER       "GhostBuster",
 12 WIT_CANNON            "Cannon",
 13 WIT_LRG				  "LavaRocksGun",
};
...
functions:
  void Precache(void) {
    ...
    switch (m_EwitType) {
	  ...
	  case WIT_LRG:				CPlayerWeapons_Precache(1<<(INDEX(WEAPON_LRG			)-1)); break;
    }
  }
  ...
  void RenderParticles(void) {
    ...
    switch (m_EwitType) {
	  ...
	  case WIT_LRG:	             Particles_Atomic(this, 1.5f, 1.5f, PT_STAR07, 12);  break;
    }
  }
  ...
  void SetProperties(void)
  {
    ...
    // *********** GRENADE LAUNCHER ***********
      case WIT_GRENADELAUNCHER:
        ...
        AddItemAttachment(GRENADELAUNCHERITEM_ATTACHMENT_MAIN_PART, MODEL_GL_MAIN_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);                   
        AddItemAttachment(GRENADELAUNCHERITEM_ATTACHMENT_SECOND_PART, MODEL_GL_SECOND_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddItemAttachment(GRENADELAUNCHERITEM_ATTACHMENT_THRID_PART, MODEL_GL_THRID_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);  
		AddItemAttachment(GRENADELAUNCHERITEM_ATTACHMENT_LAST_PART, MODEL_GL_LAST_PART, TEXTURE_GL_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);     
        StretchItem( bDM ? vDMStretch : (FLOAT3D(2.5f, 2.5f, 2.5f)));
        break;
    ...
    // *********** CANNON ***********
      case WIT_CANNON:
		...
		AddItemAttachment(CANNON_ATTACHMENT_FUSE, MODEL_CN_FUSE, TEXTURE_FUSE, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        StretchItem( bDM ? vDMStretch : (FLOAT3D(3.0f, 3.0f, 3.0f)));
        break;
	...
	// *********** LRG ***********
      case WIT_LRG:
        m_fRespawnTime = 30.0f; 
        m_strDescription.PrintF("Lava Rocks Gun");
        AddItem(MODEL_LRG, TEXTURE_BODY, 0, 0, 0);
        AddItemAttachment(LRGITEM_ATTACHMENT_BODY, MODEL_LR_BODY, TEXTURE_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddItemAttachment(LRGITEM_ATTACHMENT_BARREL, MODEL_LR_BARREL, TEXTURE_BARREL, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        StretchItem( bDM ? vDMStretch : (FLOAT3D(3.0f, 3.0f, 3.0f)));
        break;
    }
      // add flare
