...
static FLOAT hud_fTextScaling2 = 1.0f;
...
void CGame::InitInternal( void)
{
  ...
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowBuild;",  &hud_bShowBuild);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowClock;",  &hud_bShowClock);
  _pShell->DeclareSymbol("persistent user FLOAT hud_fTextScaling2;",  &hud_fTextScaling2);
...
static void PrintStats( CDrawPort *pdpDrawPort)
{
  ...
  if( hud_bShowResolution) {
    ...
    pdpDrawPort->SetTextScaling( fTextScale*hud_fTextScaling2);
    ...
    // display build ver
  if( hud_bShowBuild) {
    CTString strVer = "^iAlpha Remake\n    b184";
    pdpDrawPort->SetFont( _pfdDisplayFont);
    pdpDrawPort->SetTextScaling( fTextScale*hud_fTextScaling2*0.85f);
    pdpDrawPort->SetTextAspect( 1.0f);
    pdpDrawPort->PutTextR( strVer, slDPWidth*0.97f, slDPHeight*0.025f, C_WHITE|255);
  }
  ...
  if( hud_bShowTime) {
    ...
    pdpDrawPort->PutTextC( strTime, slDPWidth*0.5f, slDPHeight*0.025f, C_WHITE|CT_OPAQUE);
  }
  ...
  { // determine and clamp size
    ...
    for( INDEX i=0; i<ctLines; i++) {
      ...
      COLOR colLine = C_YELLOW;                 
           if( nge==NGET_ACTION)            colLine = C_YELLOW;  // normal action (default)
      ...
      else if( nge==NGET_SKIPPEDACTION)     colLine = C_GREEN; // skipped action
  ...
  CTString strFPS = "?";
       if( fFPS >= 30)   strFPS.PrintF( "%3.0f FPS", fFPS);
  else if( fFPS >= 0.1f) strFPS.PrintF( "%3.1f FPS", fFPS);
  ...
  { // display extensive statistics
    ...
    pdpDrawPort->PutText( strReport, 4, 65, C_YELLOW|CT_OPAQUE);
  }
}
...
void CGame::LCDInit(void)
{
  try {
    ...
    _toBackdrop.SetData_t(CTFILENAME("Textures\\General\\MenuBack.tex"));
...
void CGame::LCDRenderClouds1(void)
{
  ...
  _pdp_SE->PutTexture(&_toBackdrop, _boxScreen_SE, C_lBLUE|255);

}
void CGame::LCDRenderClouds2(void)
{
  LerpColor( C_WHITE, C_BLACK, sin(_tmNow_SE*10.0f)*0.5f+0.5f);
}
...
COLOR CGame::LCDGetColor(COLOR colDefault, const char *strName)
{
  ...
  PIX scrW  = _pdp_SE->GetWidth();
  PIX scrH = _pdp_SE->GetHeight();
  FLOAT pixW = (FLOAT)scrW / 640.0f;
  FLOAT pixH = (FLOAT)scrH / 480.0f;

  if (!strcmp(strName, "thumbnail border")) {
    colDefault = SE_COL_YELLOW|255;
  } else if (!strcmp(strName, "no thumbnail")) {
    colDefault = SE_COL_DARKYELLOW|255;
  } else if (!strcmp(strName, "popup box")) {
    colDefault = SE_COL_YELLOW|255;
  } else if (!strcmp(strName, "tool tip")) {
	  _pdp_SE->SetTextScaling(pixW * 0.75f);
    colDefault = SE_COL_YELLOW|255;
    ...
    colDefault = SE_COL_DARKYELLOW|75;
...
{
if (!IsScriptSoundPlaying(14)) { 
PlayScriptSound(14, CTFILENAME("Music\\Menu\\MenuBcg.ogg"), 6.0f*gam_fChatSoundVolume, 1.0f, FALSE);
}
}
