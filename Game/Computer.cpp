...
void PrintTitle(CDrawPort *pdp)
{
  SetFont2(pdp);
  CTString strTitle;
  strTitle.PrintF(TRANS("Y-KNOT 1.01 - personal version for: %s"), 
...
void CGame::ComputerRender(CDrawPort *pdp)
{
  ...
  _colMedium = LCDFadedColor(C_YELLOW|255);
  _colDark   = LCDFadedColor(LerpColor(C_dYELLOW, C_YELLOW, 0.5f)|255);
  _colBoxes  = LCDFadedColor(LerpColor(C_dYELLOW, C_YELLOW, 0.5f)|255);

  // background
  LCDRenderConsoleBcg();
  LCDRenderCloudsFaded();
