...
// console variables
static const FLOAT tmConsoleFade   = 1.0f;  // how many seconds it takes console to fade in/out
...
void CGame::ConsoleRender(CDrawPort *pdp)
{
  ...
  COLOR colDark  = LCDFadedColor(C_YELLOW|255);
  ...
  LCDRenderConsoleBcg();
  dpConsole.DrawLine( 0, pixSizeJ-1, pixSizeI, pixSizeJ-1, LCDFadedColor(C_YELLOW|255));
...
void CGame::ConsolePrintLastLines(CDrawPort *pdp)
{
  ...
  for( INDEX iLine=0; iLine<ctLines; iLine++) {
    ...
    pdp->PutText( strLine, 0, pixCharHeight*(ctLines-iLine-1), C_WHITE|255);
  }
}
